...
In order to use Sbn as a CMS and build your website with it, you should not build it yourself. If you're reading this then you're trying to contribute to Sbn or you're debugging a complex issue.

- Are you about to create a pull request for Sbn?
- Are you trying to get to the bottom of a problem in your existing Sbn installation?

If the answer is yes, please read on. Otherwise, make sure to head on over [to the download page](https://our.sbn.com/download) and start using Sbn CMS as intended.
...
[Building from source](#building-from-source)
  ...
  * [Build infrastructure](#build-infrastructure)
    ...
    * [GetSbnVersion](#getsbnversion)
    * [SetSbnVersion](#setsbnversion)
    * [Build](#build)
    * [Build-SbnDocs](#build-sbndocs)
...
To build Sbn, fire up PowerShell and move to Sbn's repository root (the directory that contains `src`, `build`, `LICENSE.md`...). There, trigger the build with the following command:
...
The Sbn Build infrastructure relies on a PowerShell object. The object can be retrieved with:

    $ubuild = build/build.ps1 -get

The object exposes various properties and methods that can be used to fine-grain build Sbn. Some, but not all, of them are detailed below.
...
Gets an object representing the current Sbn version. Example:

    $v = $ubuild.GetSbnVersion()
    Write-Host $v.Semver
...
Modifies Sbn files with the new version.

>This entirely replaces the legacy `SbnVersion.txt` file. Do *not* edit version infos in files.
...
The version must be a valid semver version. It can include a *pre release* part (eg `alpha02`) and/or a *build number* (eg `1234`). Examples:
    ...
    $ubuild.SetSbnVersion("7.6.33")
    $ubuild.SetSbnVersion("7.6.33-alpha.2")
    $ubuild.SetSbnVersion("7.6.33+1234")
    $ubuild.SetSbnVersion("7.6.33-beta.5+5678")

#### Build

Builds Sbn. Temporary files are generated in `build.tmp` while the actual artifacts (zip files, NuGet packages...) are produced in `build.out`. Example:
...
Building Sbn requires a clean `web.config` file in the `Sbn.Web.UI` project. If a `web.config` file already exists, the `pre-build` task (see below) will save it as `web.config.temp-build` and replace it with a clean copy of `web.Template.config`. The original file is replaced once it is safe to do so, by the `pre-packages` task.

#### Build-SbnDocs

Builds sbn documentation. Temporary files are generated in `build.tmp` while the actual artifacts (docs...) are produced in `build.out`. Example:
...
At the very minimum, you want
    ...
    git clean -Xdf src/Sbn.Web.UI/App_Data
    rm src/Sbn.Web.UI/web.config
...
Sbn uses Azure DevOps for continuous integration, nightly builds and release builds. The Sbn CMS project on DevOps [is available for anonymous users](https://sbn.visualstudio.com/Sbn%20Cms).

DevOps uses the `Build-Sbn` command several times, each time passing a different *target* parameter. The supported targets are:

* `pre-build`: prepares the build
* `compile-belle`: compiles Belle
* `compile-sbn`: compiles Sbn
...
All these targets are executed when `Build-Sbn` is invoked without a parameter (or with the `all` parameter). On VSTS, compilations (of Sbn and tests) are performed by dedicated DevOps tasks. Similarly, creating the NuGet packages is also performed by dedicated DevOps tasks.
...
>During a DevOps build, some environment `SBN_*` variables are exported by the `pre-build` target and can be reused in other targets *and* in DevOps tasks. The `SBN_TMP` environment variable is used in `Sbn.Tests` to disable some tests that have issues with DevOps at the moment.
