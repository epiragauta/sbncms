  ...
  # THIS FILE IS DISTRIBUTED AS PART OF SBN.BUILD
  ...
  $cache = 4
  $nuget = "$scriptTemp\nuget.exe"
  # ensure the correct NuGet-source is used. This one is used by Sbn
  $nugetsourceSbn = "https://www.myget.org/F/sbnprereleases/api/v3/index.json"
  if (-not $local)
  {
  ...
  # or above. Which means we cannot really use -ConfigFile but instead have to have Sbn's
  ...
  if (-not $local)
  {
    $params = "-OutputDirectory", $scriptTemp, "-Verbosity", "quiet", "-PreRelease", "-Source", $nugetsourceSbn
    &$nuget install Sbn.Build @params
    if (-not $?) { throw "Failed to download Sbn.Build." }
  }

  # ensure we have the build system
  $ubuildPath = ls "$scriptTemp\Sbn.Build.*" | sort -property CreationTime -descending | select -first 1
  if (-not $ubuildPath)
  {
