...
if ($project) {
	...
	# Copy sbn and sbn_files from package to project folder
	$sbnFolder = Join-Path $projectPath "Sbn"
	New-Item -ItemType Directory -Force -Path $sbnFolder	
	$sbnFolderSource = Join-Path $installPath "SbnFiles\Sbn"		

    Write-Host "copying files to $sbnFolder ..."
    # see https://support.microsoft.com/en-us/help/954404/return-codes-that-are-used-by-the-robocopy-utility-in-windows-server-2
    robocopy $sbnFolderSource $sbnFolder /is /it /e
	...
	if(Test-Path $destinationWebConfig) 
	{
		Try 
		{
			...
			$config.configuration.appSettings.ChildNodes | ForEach-Object { 
				if($_.key -eq "Sbn.Core.ConfigurationStatus") 
				{
					# The web.config has an sbn-specific appSetting in it
	...
	{
		$packageWebConfigSource = Join-Path $installPath "SbnFiles\Web.config"
		...
		$splashesSource = Join-Path $installPath "SbnFiles\Config\splashes\*.*"
		...
		$upgradeViewSource = Join-Path $sbnFolderSource "Views\install\*"
		$upgradeView = Join-Path $sbnFolder "Views\install\"
		Write-Host "Copying2 ${upgradeViewSource} to ${upgradeView}"
		Copy-Item $upgradeViewSource $upgradeView -Force
		
		Try 
		{
			# Disable tours for upgrades, presumably Sbn experience is already available
			$sbnSettingsConfigPath = Join-Path $configFolder "sbnSettings.config"
			$content = (Get-Content $sbnSettingsConfigPath).Replace('<tours enable="true">','<tours enable="false">')
			# Saves with UTF-8 encoding without BOM which makes sure Sbn can still read it
			# Reference: https://stackoverflow.com/a/32951824/5018
			[IO.File]::WriteAllLines($sbnSettingsConfigPath, $content)
		} 
		Catch 
		{
