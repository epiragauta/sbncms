...
$pattern = "SbnCms.([0-9]{1,2}.[0-9]{1,3}.[0-9]{1,3}).zip"
Write-Host "Searching for Sbn release files in $($zipsDir) for a file with pattern $($pattern)"
...
if($file)
{
    ...
    git config --global user.email "coffee@sbn.com" 2>&1 | % { $_.ToString() }
    git config --global user.name "Sbn HQ" 2>&1 | % { $_.ToString() }
    
    # Commit
    CD "$($workingDirectory)\$($env:GIT_REPOSITORYNAME)"
    Write-Host "Committing Sbn $($version) Release from Build Output"
...
{
    Write-Error "Sbn release file not found, searched in $($workingDirectory)\$($zipsDir) for a file with pattern $($pattern) - canceling"
}
