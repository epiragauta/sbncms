...
namespace Sbn.Cms.Tests.UnitTests.Sbn.Core.Services
{
    [TestFixture]
    public class UserDataServiceTests
    {
        private ISbnVersion _sbnVersion;

        [OneTimeSetUp]
        public void CreateMocks() => CreateSbnVersion(9, 0, 0);
        ...
        private UserDataService CreateUserDataService(string culture)
        {
            ...
            return new UserDataService(_sbnVersion, localizationService);
        }
        ...
        private void CreateSbnVersion(int major, int minor, int patch)
        {
            var sbnVersion = new Mock<ISbnVersion>();
            var semVersion = new SemVersion(major, minor, patch);
            sbnVersion.Setup(x => x.SemanticVersion).Returns(semVersion);
            _sbnVersion = sbnVersion.Object;
        }
    }
}
