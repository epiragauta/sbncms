...
using User = Sbn.Cms.Core.Models.Membership.User;

namespace Sbn.Cms.Tests.UnitTests.Sbn.Core.Models
{
    [TestFixture]
    public class UserExtensionsTests
    {
        ...
        public void Determines_Path_Based_Access_To_Content(int startNodeId, string startNodePath, string contentPath, bool outcome)
        {
            ...
            esmock
                .Setup(x => x.GetAllPaths(It.IsAny<SbnObjectTypes>(), It.IsAny<int[]>()))
                .Returns<SbnObjectTypes, int[]>((type, ids) => new[] { new TreeEntityPath { Id = startNodeId, Path = startNodePath } });
        ...
        public void CombineStartNodes(string groupSn, string userSn, string expected)
        {
            ...
            esmock
                .Setup(x => x.GetAllPaths(It.IsAny<SbnObjectTypes>(), It.IsAny<int[]>()))
                .Returns<SbnObjectTypes, int[]>((type, ids) => paths.Where(x => ids.Contains(x.Key)).Select(x => new TreeEntityPath { Id = x.Key, Path = x.Value }));
            ...
            var combinedA = UserExtensions.CombineStartNodes(SbnObjectTypes.Document, groupSnA, userSnA, esmock.Object).OrderBy(x => x).ToArray();
