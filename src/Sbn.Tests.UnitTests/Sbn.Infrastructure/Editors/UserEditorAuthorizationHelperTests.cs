...
namespace Sbn.Cms.Tests.UnitTests.Sbn.Infrastructure.Editors
{
    [TestFixture]
    public class UserEditorAuthorizationHelperTests
    {
        ...
        public void Can_Add_Another_Content_Start_Node_On_User_With_Access()
        {
            ...
            var userService = new Mock<IUserService>();
            var entityService = new Mock<IEntityService>();
            entityService.Setup(service => service.GetAllPaths(It.IsAny<SbnObjectTypes>(), It.IsAny<int[]>()))
                .Returns((SbnObjectTypes objType, int[] ids) => ids.Select(x => new TreeEntityPath { Path = nodePaths[x], Id = x }));
