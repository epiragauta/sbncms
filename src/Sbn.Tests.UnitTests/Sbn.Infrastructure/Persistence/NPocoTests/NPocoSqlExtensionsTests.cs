...
namespace Sbn.Cms.Tests.UnitTests.Sbn.Infrastructure.Persistence.NPocoTests
{
    ...
    public class NPocoSqlExtensionsTests : BaseUsingSqlSyntax
    {
        ...
        public void WhereTest()
        {
            ...
            Assert.AreEqual("SELECT *\nFROM [sbnPropertyData]\nWHERE (([sbnPropertyData].[languageId] is null))", sql.SQL, sql.SQL);

            sql = new Sql<ISqlContext>(SqlContext)
                .Select("*")
                .From<PropertyDataDto>()
                .Where<PropertyDataDto>(x => x.LanguageId == 123);
            Assert.AreEqual("SELECT *\nFROM [sbnPropertyData]\nWHERE (([sbnPropertyData].[languageId] = @0))", sql.SQL, sql.SQL);
            ...
            Assert.AreEqual("SELECT *\nFROM [sbnPropertyData]\nWHERE ((((@0 is null) AND ([sbnPropertyData].[languageId] is null)) OR ((@1 is not null) AND ([sbnPropertyData].[languageId] = @2))))", sql.SQL, sql.SQL);
            ...
            // Assert.AreEqual("SELECT *\nFROM [sbnPropertyData]\nWHERE ((((@0 is null) AND ([sbnPropertyData].[languageId] is null)) OR ((@0 is not null) AND ([sbnPropertyData].[languageId] = @0))))", sql.SQL, sql.SQL);
            ...
            Assert.AreEqual("SELECT *\nFROM [sbnPropertyData]\nWHERE ((COALESCE([sbnPropertyData].[languageId],@0) = COALESCE(@1,@0)))", sql.SQL, sql.SQL);
        }
        ...
        public void WhereInValueFieldTest()
        {
            ...
            Assert.AreEqual("SELECT *\nFROM [sbnNode]\nWHERE ([sbnNode].[id] IN (@0,@1,@2))", sql.SQL);
        }
        ...
        public void WhereInObjectFieldTest()
        {
            ...
            // and returned "[sbnNode].[text] = @0"... had to fix WhereIn.
            Sql<ISqlContext> sql = new Sql<ISqlContext>(SqlContext)
                .Select("*")
                .From<NodeDto>()
                .WhereIn<NodeDto>(x => x.Text, new[] { "a", "b", "c" });
            Assert.AreEqual("SELECT *\nFROM [sbnNode]\nWHERE ([sbnNode].[text] IN (@0,@1,@2))", sql.SQL);
        }
...
INNER JOIN [dto2] ON [dto1].[id] = [dto2].[dto1id]".NoCrLf(), sql.SQL.NoCrLf());
        ...
        {
            Sql<ISqlContext> sql = Sql()
                .Update<DataTypeDto>(u => u.Set(x => x.EditorAlias, "Sbn.ColorPicker"))
                .Where<DataTypeDto>(x => x.EditorAlias == "Sbn.ColorPickerAlias");
        }
