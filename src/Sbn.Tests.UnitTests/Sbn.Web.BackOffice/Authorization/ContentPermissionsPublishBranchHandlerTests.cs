...
namespace Sbn.Cms.Tests.UnitTests.Sbn.Web.BackOffice.Authorization
{
    public class ContentPermissionsPublishBranchHandlerTests
    {
        ...
        private static Mock<IEntityService> CreateMockEntityService()
        {
            ...
            mockEntityService
                .Setup(x => x.GetPagedDescendants(It.Is<int>(y => y == NodeId), It.Is<SbnObjectTypes>(y => y == SbnObjectTypes.Document), It.IsAny<long>(), It.IsAny<int>(), out totalRecords, It.IsAny<IQuery<ISbnEntity>>(), It.IsAny<Ordering>()))
