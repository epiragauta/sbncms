...
namespace Sbn.Cms.Tests.UnitTests.Sbn.Web.Common.ModelBinders
{
    [TestFixture]
    public class ContentModelBinderTests
    {
        ...
        [Test]
        public async Task Does_Not_Bind_Model_When_SbnToken_Not_In_Route_Values()
        {
            // Arrange
            IPublishedContent pc = CreatePublishedContent();
            ModelBindingContext bindingContext = CreateBindingContextForSbnRequest(typeof(ContentModel), pc);
            bindingContext.ActionContext.HttpContext.Features.Set<SbnRouteValues>(null);
        ...
        [Test]
        public async Task Bind_Model_When_SbnToken_Is_In_Route_Values()
        {
            // Arrange
            IPublishedContent pc = CreatePublishedContent();
            ModelBindingContext bindingContext = CreateBindingContextForSbnRequest(typeof(ContentModel), pc);
        ...
        /// <summary>
        /// Creates a binding context with the route values populated to similute an Sbn dynamically routed request
        /// </summary>
        private ModelBindingContext CreateBindingContextForSbnRequest(Type modelType, IPublishedContent publishedContent)
        {
            ...
            httpContext.Features.Set(new SbnRouteValues(publishedRequest, null));
