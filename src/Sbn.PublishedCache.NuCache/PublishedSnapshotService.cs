...
namespace Sbn.Cms.Infrastructure.PublishedCache
{
    internal class PublishedSnapshotService : IPublishedSnapshotService
    {
        ...
        // SetSbnVersionStep issues a DistributedCache.Instance.RefreshAll...() call which should cause
        // the entire content, media etc caches to reload from database -- and then the app restarts -- however,
        // at the time SetSbnVersionStep runs, Sbn is not fully initialized and therefore some property
        ...
        // cache, and XmlStore just writes a new sbn.config file upon RefreshAll, so that's OK.
        //
        ...
        public void Notify(ContentTypeCacheRefresher.JsonPayload[] payloads)
        {
            ...
            if (_publishedModelFactory.IsLiveFactoryEnabled())
            {
                ...
                // see https://github.com/sbn/Sbn-CMS/issues/5671
        ...
        public PublishedSnapshot.PublishedSnapshotElements GetElements(bool previewDefault)
        {
            ...
            lock (_elementsLock)
            {
                ...
                else
                {
                    ...
                    scopeContext.Enlist("Sbn.Web.PublishedCache.NuCache.PublishedSnapshotService.Resync", () => this, (completed, svc) =>
                    {
