...
using static Sbn.Cms.Core.Persistence.SqlExtensionsStatics;
using Constants = Sbn.Cms.Core.Constants;

namespace Sbn.Cms.Infrastructure.PublishedCache.Persistence
{
    ...
    AND {Constants.DatabaseSchema.Tables.Content}.contentTypeId IN (@ctypes)
        ...
        public bool VerifyContentDbCache()
        {
            ...
            var count = Database.ExecuteScalar<int>(
                $@"SELECT COUNT(*)
FROM sbnNode
JOIN {Constants.DatabaseSchema.Tables.Document} ON sbnNode.id={Constants.DatabaseSchema.Tables.Document}.nodeId
LEFT JOIN cmsContentNu nuEdited ON (sbnNode.id=nuEdited.nodeId AND nuEdited.published=0)
LEFT JOIN cmsContentNu nuPublished ON (sbnNode.id=nuPublished.nodeId AND nuPublished.published=1)
WHERE sbnNode.nodeObjectType=@objType
AND nuEdited.nodeId IS NULL OR ({Constants.DatabaseSchema.Tables.Document}.published=1 AND nuPublished.nodeId IS NULL);",
        ...
        public bool VerifyMediaDbCache()
        {
            ...
            var count = Database.ExecuteScalar<int>(
                @"SELECT COUNT(*)
FROM sbnNode
LEFT JOIN cmsContentNu ON (sbnNode.id=cmsContentNu.nodeId AND cmsContentNu.published=0)
WHERE sbnNode.nodeObjectType=@objType
...
", new { objType = memberObjectType });
        ...
        private Sql<ISqlContext> SqlContentSourcesSelectSbnNodeJoin(ISqlContext sqlContext)
        {
            var syntax = sqlContext.SqlSyntax;

            var sqlTemplate = sqlContext.Templates.Get(Constants.SqlTemplates.NuCacheDatabaseDataSource.SourcesSelectSbnNodeJoin, builder =>
        ...
        {
            var sql = SqlContentSourcesSelect(SqlContentSourcesSelectSbnNodeJoin)
                .Append(SqlObjectTypeNotTrashed(SqlContext, Constants.ObjectTypes.Document))
                .Append(SqlWhereNodeIdX(SqlContext, id))
                .Append(SqlOrderByLevelIdSortOrder(SqlContext));

            // Use a more efficient COUNT query
            var sqlCountQuery = SqlContentSourcesCount(SqlContentSourcesSelectSbnNodeJoin)
                .Append(SqlObjectTypeNotTrashed(SqlContext, Constants.ObjectTypes.Document))
                .Append(SqlWhereNodeIdX(SqlContext, id));
            var sqlCount = SqlContext.Sql("SELECT COUNT(*) FROM (").Append(sqlCountQuery).Append(") npoco_tbl");
        ...
        {
            var sql = SqlMediaSourcesSelect(SqlContentSourcesSelectSbnNodeJoin)
                .Append(SqlObjectTypeNotTrashed(SqlContext, Constants.ObjectTypes.Media))
                .Append(SqlWhereNodeIdX(SqlContext, id))
                .Append(SqlOrderByLevelIdSortOrder(SqlContext));

            // Use a more efficient COUNT query
            var sqlCountQuery = SqlMediaSourcesCount(SqlContentSourcesSelectSbnNodeJoin)
                .Append(SqlObjectTypeNotTrashed(SqlContext, Constants.ObjectTypes.Media))
                .Append(SqlWhereNodeIdX(SqlContext, id));
            var sqlCount = SqlContext.Sql("SELECT COUNT(*) FROM (").Append(sqlCountQuery).Append(") npoco_tbl");
