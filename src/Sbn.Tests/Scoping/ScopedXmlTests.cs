...
namespace Sbn.Tests.Scoping
{
    ...
    public class ScopedXmlTests : TestWithDatabaseBase
    {
        ...
        private static XmlDocument XmlInContext => ((PublishedContentCache) Sbn.Web.Composing.Current.SbnContext.Content).GetXml(false);

        [TestCase(true)]
        [TestCase(false)]
        public void TestScope(bool complete)
        {
            var sbnContext = GetSbnContext("http://example.com/", setSingleton: true);

            // sanity checks
            Assert.AreSame(sbnContext, Sbn.Web.Composing.Current.SbnContext);
            Assert.AreSame(XmlStore, ((PublishedContentCache) sbnContext.Content).XmlStore);
