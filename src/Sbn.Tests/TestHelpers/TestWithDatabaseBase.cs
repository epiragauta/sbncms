...
{
    /// <summary>
    /// Provides a base class for all Sbn tests that require a database.
    /// </summary>
    /// <remarks>
    /// <para>Can provide a SqlCE database populated with the Sbn schema. The database should
    /// be accessed through the SbnDatabaseFactory.</para>
    /// <para>Provides an Sbn context and Xml content.</para>
    /// <para>fixme what else?</para>
    /// </remarks>
    [SbnTest(WithApplication = true)]
    public abstract class TestWithDatabaseBase : SbnTestBase
    {
        ...
        internal ISbnDatabaseFactory SbnDatabaseFactory => Factory.GetRequiredService<ISbnDatabaseFactory>();
        ...
        protected override void Compose()
        {
            ...
            Builder.WithCollectionBuilder<SbnApiControllerTypeCollectionBuilder>()
                .Add(Builder.TypeLoader.GetSbnApiControllers());

            Builder.Services.AddUnique(f =>
            {
                if (Options.Database == SbnTestOptions.Database.None)
                ...
                var factory = new SbnDatabaseFactory(f.GetRequiredService<ILogger<SbnDatabaseFactory>>(), f.GetRequiredService<ILoggerFactory>(), GetDbConnectionString(), GetDbProviderName(), lazyMappers, TestHelper.DbProviderFactoryCreator, f.GetRequiredService<DatabaseSchemaCreatorFactory>());
                return factory;
            });
        }
        ...
        public override void TearDown()
        {
            ...
            try
            {
                ...
                if (Options.Database == SbnTestOptions.Database.NewSchemaPerTest)
                    RemoveDatabaseFile();
        ...
        {
            return @"DataSource=|DataDirectory|SbnNPocoTests.sdf;Flush Interval=1;";
        }
        ...
        protected virtual void CreateSqlCeDatabase()
        {
            ...
            var settings = ConfigurationManager.ConnectionStrings[Constants.System.SbnConnectionName];
            ConfigurationManager.AppSettings.Set(
                Constants.System.SbnConnectionName,
                GetDbConnectionString());

            _databasePath = string.Concat(path, "\\SbnNPocoTests.sdf");
            ...
            if (FirstTestInSession
                ...
                || Options.Database == SbnTestOptions.Database.NewSchemaPerTest
                || Options.Database == SbnTestOptions.Database.NewEmptyPerTest
                || (FirstTestInFixture && Options.Database == SbnTestOptions.Database.NewSchemaPerFixture))
            {
                ...
                {
                    if (Options.Database != SbnTestOptions.Database.NewEmptyPerTest && _databaseBytes != null)
                    {
        ...
        protected virtual IPublishedSnapshotService CreatePublishedSnapshotService(GlobalSettings globalSettings = null)
        {
            ...
            var publishedSnapshotAccessor = new SbnContextPublishedSnapshotAccessor(Sbn.Web.Composing.Current.SbnContextAccessor);
            ...
            var service = new XmlPublishedSnapshotService(
                ...
                Factory.GetRequiredService<ISbnContextAccessor>(),
        ...
        {
            if (Options.Database == SbnTestOptions.Database.None || Options.Database == SbnTestOptions.Database.NewEmptyPerTest)
                return;
                ...
                || FirstTestInFixture && Options.Database == SbnTestOptions.Database.NewSchemaPerFixture))
            {
                ...
                {
                    var schemaHelper = new DatabaseSchemaCreator(scope.Database, LoggerFactory.CreateLogger<DatabaseSchemaCreator>(), LoggerFactory, SbnVersion, Mock.Of<IEventAggregator>());
                    //Create the sbn database and its base data
        ...
        private void CloseDbConnections(ISbnDatabase database)
        {
        ...
        private void RemoveDatabaseFile(ISbnDatabase database, Action<Exception> onFail = null)
        {
        ...
        private void RemoveDatabaseFile(Action<Exception> onFail = null)
        {
            ...
            try
            {
                var filePath = string.Concat(path, "\\SbnNPocoTests.sdf");
        ...
        protected ISbnContextAccessor GetSbnContextAccessor(ISbnContext ctx) => new TestSbnContextAccessor(ctx);

        protected ISbnContext GetSbnContext(string url, int templateId = 1234, RouteData routeData = null, bool setSingleton = false, GlobalSettings globalSettings = null, IPublishedSnapshotService snapshotService = null)
        {
