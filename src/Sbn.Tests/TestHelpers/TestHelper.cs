...
namespace Sbn.Tests.TestHelpers
{
    ...
    public static class TestHelper
    {
        ...
        private class TestHelperInternal : TestHelperBase
        {
            ...
            public override IDbProviderFactoryCreator DbProviderFactoryCreator { get; } = new SbnDbProviderFactoryCreator();
            public DatabaseSchemaCreatorFactory DatabaseSchemaCreatorFactory { get; } = new DatabaseSchemaCreatorFactory(Mock.Of<ILogger<DatabaseSchemaCreator>>(), NullLoggerFactory.Instance, new SbnVersion(), Mock.Of<IEventAggregator>());
        ...
        {
            CreateDirectories(new[] { Constants.SystemDirectories.MvcViews, new GlobalSettings().SbnMediaPath, Constants.SystemDirectories.AppPlugins });
        }

        public static void CleanContentDirectories()
        {
            CleanDirectories(new[] { Constants.SystemDirectories.MvcViews, new GlobalSettings().SbnMediaPath });
        }
        ...
        public static void CleanSbnSettingsConfig()
        {
            var currDir = new DirectoryInfo(WorkingDirectory);

            var sbnSettingsFile = Path.Combine(currDir.Parent.Parent.FullName, "config", "sbnSettings.config");
            if (File.Exists(sbnSettingsFile))
                File.Delete(sbnSettingsFile);
        }
        ...
        public static ISbnVersion GetSbnVersion() => _testHelperInternal.GetSbnVersion();
        ...
        public static IServiceProvider CreateServiceProvider(ISbnBuilder builder)
        {
            builder.Build();
