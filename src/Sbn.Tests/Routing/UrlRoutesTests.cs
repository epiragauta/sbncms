...
DetermineRouteById(id):
    ...
    return route
        ...
        public void GetRouteByIdNoHide(int id, bool hide, string expected)
        {
            ...
            var sbnContext = GetSbnContext("/test", 0, globalSettings: globalSettings);
        ...
        public void GetRouteByIdHide(int id, bool hide, string expected)
        {
            ...
            var sbnContext = GetSbnContext("/test", 0, globalSettings: globalSettings, snapshotService: snapshotService);
            var cache = sbnContext.Content as PublishedContentCache;
            if (cache == null) throw new Exception("Unsupported IPublishedContentCache, only the Xml one is supported.");
