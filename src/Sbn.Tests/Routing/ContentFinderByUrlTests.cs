...
namespace Sbn.Tests.Routing
{
    ...
    public class ContentFinderByUrlTests : BaseWebTest
    {
        ...
        public async Task Match_Document_By_Url_Hide_Top_Level(string urlString, int expectedId)
        {
            ...
            var sbnContext = GetSbnContext(urlString, globalSettings:globalSettings, snapshotService: snapshotService);
        ...
        public async Task Match_Document_By_Url(string urlString, int expectedId)
        {
            ...
            var sbnContext = GetSbnContext(urlString, globalSettings:globalSettings);
            var publishedRouter = CreatePublishedRouter(GetSbnContextAccessor(sbnContext));
            var frequest = await publishedRouter .CreateRequestAsync(sbnContext.CleanedSbnUrl);
            var lookup = new ContentFinderByUrl(LoggerFactory.CreateLogger<ContentFinderByUrl>(), GetSbnContextAccessor(sbnContext));
