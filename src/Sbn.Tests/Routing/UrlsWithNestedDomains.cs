...
namespace Sbn.Tests.Routing
{
    ...
    public class UrlsWithNestedDomains : UrlRoutingTestBase
    {
        ...
        public async Task DoNotPolluteCache()
        {
            ...
            var sbnContext = GetSbnContext(url, 9999, globalSettings: globalSettings);
            var sbnContextAccessor = GetSbnContextAccessor(sbnContext);
            ...
            // route a rogue URL
            var publishedRouter = CreatePublishedRouter(sbnContextAccessor);
            ...
            var lookup = new ContentFinderByUrl(LoggerFactory.CreateLogger<ContentFinderByUrl>(), GetSbnContextAccessor(sbnContext));
        ...
        private IPublishedUrlProvider GetPublishedUrlProvider(ISbnContext sbnContext, object urlProvider)
        {
        ...
        void SetDomains1()
        {
            SetupDomainServiceMock(new[]
            {
                ...
                new SbnDomain("http://domain2.com/") {Id = 1, LanguageId = LangEngId, RootContentId = 10011, LanguageIsoCode = "en-US"}
            });
