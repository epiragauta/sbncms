...
namespace Sbn.Tests.Routing
{
    ...
    public class GetContentUrlsTests : UrlRoutingTestBase
    {
        ...
        public async Task Content_Not_Published()
        {
            ...
            var umbContext = GetSbnContext("http://localhost:8000");
            var publishedRouter = CreatePublishedRouter(
                GetSbnContextAccessor(umbContext),
                Factory,
                contentFinders: new ContentFinderCollection(new[] { new ContentFinderByUrl(LoggerFactory.CreateLogger<ContentFinderByUrl>(), GetSbnContextAccessor(umbContext)) }));
        ...
        public async Task Invariant_Root_Content_Published_No_Domains()
        {
            ...
            var umbContext = GetSbnContext("http://localhost:8000");
            var sbnContextAccessor = GetSbnContextAccessor(umbContext);
            ...
            var publishedRouter = CreatePublishedRouter(
                sbnContextAccessor,
                Factory,
                contentFinders:new ContentFinderCollection(new[]{new ContentFinderByUrl(LoggerFactory.CreateLogger<ContentFinderByUrl>(), sbnContextAccessor) }));
            var urls = (await content.GetContentUrlsAsync(publishedRouter,
                umbContext,
                GetLangService("en-US", "fr-FR"), GetTextService(), ServiceContext.ContentService,
                VariationContextAccessor,
