...
namespace Sbn.Tests.Routing
{
    ...
    public class ContentFinderByAliasWithDomainsTests : ContentFinderByAliasTests
    {
        ...
        public async Task Lookup_By_Url_Alias_And_Domain(string inputUrl, string expectedCulture, int expectedNode)
        {
            ...
            var sbnContext = GetSbnContext(inputUrl);
            ...
            var finder = new ContentFinderByUrlAlias(LoggerFactory.CreateLogger<ContentFinderByUrlAlias>(), Mock.Of<IPublishedValueFallback>(), VariationContextAccessor, GetSbnContextAccessor(sbnContext));
