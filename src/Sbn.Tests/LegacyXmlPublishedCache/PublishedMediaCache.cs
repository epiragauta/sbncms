...
namespace Sbn.Tests.LegacyXmlPublishedCache
{
    ...
    internal class PublishedMediaCache : PublishedCacheBase, IPublishedMediaCache
    {
        ...
        public PublishedMediaCache(XmlStore xmlStore, IMediaService mediaService, IUserService userService,
            ...
            ISbnContextAccessor sbnContextAccessor, IVariationContextAccessor variationContextAccessor)
            : base(false)
        {
        ...
        internal PublishedMediaCache(IMediaService mediaService, IUserService userService, ISearcher searchProvider, IAppCache appCache, PublishedContentTypeCache contentTypeCache, IEntityXmlSerializer entitySerializer, ISbnContextAccessor sbnContextAccessor)
            : base(false)
        {
            _mediaService = mediaService ?? throw new ArgumentNullException(nameof(mediaService));
            _userService = userService ?? throw new ArgumentNullException(nameof(userService));
        ...
        {
            return GetSbnMedia(contentId) != null;
        }
        ...
        public override IEnumerable<IPublishedContent> GetAtRoot(bool preview, string culture = null)
        {
            ...
            if (searchProvider != null)
            {
                try
                {
                    ...
                    var filter = criteria.ParentId(-1).Not().Field(SbnExamineFieldNames.IndexPathFieldName, "-1,-21,".MultipleCharacterWildcard());
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        //Currently examine is throwing FileNotFound exceptions when we have a load balanced filestore and a node is published in sbn
            ...
            return rootMedia.Select(m => GetSbnMedia(m.Id));
        }
        ...
        private IPublishedContent GetSingleByXPath(XPathNodeIterator iterator)
        {
            ...
            //int id;
            //return int.TryParse(idAttr, out id) ? GetSbnMedia(id) : null;
        }
        ...
        private ISearcher GetSearchProviderSafe()
        {
            ...
            try
            {
                return _examineManager.TryGetIndex(Constants.SbnIndexes.InternalIndexName, out var index) ? index.GetSearcher() : null;
            }
        ...
        private IPublishedContent GetSbnMedia(int id)
        {
            ...
            var cacheValues = GetCacheValues(id, GetSbnMediaCacheValues);

            return cacheValues == null ? null : CreateFromCacheValues(cacheValues);
        }

        private CacheValues GetSbnMediaCacheValues(int id)
        {
            var searchProvider = GetSearchProviderSafe();
            ...
            if (searchProvider != null)
            {
                try
                {
                    ...
                    var criteria = searchProvider.CreateQuery("media");
                    var filter = criteria.Id(id.ToInvariantString()).Not().Field(SbnExamineFieldNames.IndexPathFieldName, "-1,-21,".MultipleCharacterWildcard());
        ...
        private IPublishedProperty GetProperty(DictionaryPublishedContent dd, string alias)
        {
            ...
            if (dd.LoadedFromExamine)
            {
                ...
                var rawValue = dd.Properties.FirstOrDefault(x => x.Alias.InvariantEquals(SbnExamineFieldNames.RawFieldPrefix + alias));
                return rawValue
        ...
        private IEnumerable<IPublishedContent> GetChildrenMedia(int parentId, XPathNavigator xpath = null)
        {
            ...
            if (searchProvider != null)
            {
                try
                {
                    ...
                    var filter = criteria.ParentId(parentId).Not().Field(SbnExamineFieldNames.IndexPathFieldName, "-1,-21,".MultipleCharacterWildcard())
        ...
        internal IPublishedContent CreateFromCacheValues(CacheValues cacheValues)
        {
            var content = new DictionaryPublishedContent(
                cacheValues.Values,
                parentId => parentId < 0 ? null : GetSbnMedia(parentId),
                GetChildrenMedia,
                GetProperty,
                _appCache,
