...
namespace Sbn.Cms.Web.Common.Templates
{
    /// <summary>
    ...
    internal class TemplateRenderer : ITemplateRenderer
    {
        ...
        public async Task RenderAsync(int pageId, int? altTemplateId, StringWriter writer)
        {
            ...
            // important to use CleanedSbnUrl - lowercase path-only version of the current URL, though this isn't going to matter
            ...
            var requestBuilder = await _publishedRouter.CreateRequestAsync(sbnContext.CleanedSbnUrl);

            var doc = sbnContext.Content.GetById(pageId);
            ...
            // in some cases the SbnContext will not have a PublishedRequest assigned to it if we are not in the
            ...
            if (sbnContext.PublishedRequest == null)
            {
            ...
            {
                requestBuilder.SetCulture(sbnContext.PublishedRequest.Culture);
            }
        ...
        // TODO: I feel like we need to do more than this, pretty sure we need to replace the SbnRouteValues
        ...
        private void SetNewItemsOnContextObjects(IPublishedRequest request)
        {
            ...
            sbnContext.PublishedRequest = request;
        }
        ...
        private void SaveExistingItems(out IPublishedRequest oldPublishedRequest)
        {
            ...
            oldPublishedRequest = sbnContext.PublishedRequest;
        }
        ...
        {
            var sbnContext = _sbnContextAccessor.GetRequiredSbnContext();
            sbnContext.PublishedRequest = oldPublishedRequest;
        }
    }
}
