...
namespace Sbn.Cms.Web.Common.Security
{
    ...
    public abstract class SbnSignInManager<TUser> : SignInManager<TUser>
        where TUser : SbnIdentityUser
    {
        // borrowed from https://github.com/dotnet/aspnetcore/blob/master/src/Identity/Core/src/SignInManager.cs
        protected const string SbnSignInMgrLoginProviderKey = "LoginProvider";
        // borrowed from https://github.com/dotnet/aspnetcore/blob/master/src/Identity/Core/src/SignInManager.cs
        protected const string SbnSignInMgrXsrfKey = "XsrfId";

        public SbnSignInManager(
        ...
        public override async Task<ExternalLoginInfo> GetExternalLoginInfoAsync(string expectedXsrf = null)
        {
            ...
            if (!items.ContainsKey(SbnSignInMgrLoginProviderKey))
            {
                throw new InvalidOperationException($"The external login authenticated successfully but the key {SbnSignInMgrLoginProviderKey} was not found in the authentication properties. Ensure you call SignInManager.ConfigureExternalAuthenticationProperties before issuing a ChallengeResult.");
            }

            if (expectedXsrf != null)
            {
                if (!items.ContainsKey(SbnSignInMgrXsrfKey))
                {
                    return null;
                }
                var userId = items[SbnSignInMgrXsrfKey];
            ...
            if (providerKey == null || items[SbnSignInMgrLoginProviderKey] is not string provider)
            {
                return null;
            }
        ...
        public override async Task SignInWithClaimsAsync(TUser user, AuthenticationProperties authenticationProperties, IEnumerable<Claim> additionalClaims)
        {
            ...
            // own ConfigureSbnBackOfficeCookieOptions which assigns the current HttpContext.User to the IPrincipal created
