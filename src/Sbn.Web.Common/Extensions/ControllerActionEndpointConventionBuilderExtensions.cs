            ...
            => builder.Add(convention =>
            {
                ...
                if (convention.Metadata.OfType<ISuppressMatchingMetadata>().FirstOrDefault()?.SuppressMatching != true)
                {
                    ...
                    if (actionDescriptor != null)
                    {
                        ...
                        var filter = new SbnVirtualPageFilterAttribute();
                        ...
                        // is being routed with ForSbnPage. In that case, ForSbnPage wins
                        // because the SbnVirtualPageFilterAttribute will check for the metadata first since
                        // that is more explicit and flexible in case the same controller is routed multiple times.
                        if (!actionDescriptor.FilterDescriptors.Any(x => x.Filter is SbnVirtualPageFilterAttribute))
                        {
