...
namespace Sbn.Extensions
{
    public static class PublishedContentExtensions
    {
        ...
        /// <param name="sbnContextAccessor"></param>
        ...
        /// </remarks>
        public static string GetCultureFromDomains(this IPublishedContent content, ISbnContextAccessor sbnContextAccessor, ISiteDomainMapper siteDomainHelper, Uri current = null)
        {
            var sbnContext = sbnContextAccessor.GetRequiredSbnContext();
            return DomainUtilities.GetCultureFromDomains(content.Id, content.Path, current, sbnContext, siteDomainHelper);
        }
        ...
        public static IEnumerable<PublishedSearchResult> SearchDescendants(this IPublishedContent content, IExamineManager examineManager, ISbnContextAccessor sbnContextAccessor, string term, string indexName = null)
        {
            indexName = string.IsNullOrEmpty(indexName) ? Constants.SbnIndexes.ExternalIndexName : indexName;
            ...
            var query = index.Searcher.CreateQuery()
                .Field(SbnExamineFieldNames.IndexPathFieldName, (content.Path + ",").MultipleCharacterWildcard())
                .And()
                .ManagedQuery(term);
            var sbnContext = sbnContextAccessor.GetRequiredSbnContext();
            return query.Execute().ToPublishedSearchResults(sbnContext.Content);
        }

        public static IEnumerable<PublishedSearchResult> SearchChildren(this IPublishedContent content, IExamineManager examineManager, ISbnContextAccessor sbnContextAccessor, string term, string indexName = null)
        {
            indexName = string.IsNullOrEmpty(indexName) ? Constants.SbnIndexes.ExternalIndexName : indexName;
            if (!examineManager.TryGetIndex(indexName, out var index))
            {
