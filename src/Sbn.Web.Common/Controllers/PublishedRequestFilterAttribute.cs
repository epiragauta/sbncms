...
{
    /// <summary>
    /// Deals with custom headers for the sbn request
    /// </summary>
    internal class PublishedRequestFilterAttribute : ResultFilterAttribute
    {
        ...
        /// </summary>
        protected SbnRouteValues GetSbnRouteValues(ResultExecutingContext context)
        {
            SbnRouteValues routeVals = context.HttpContext.Features.Get<SbnRouteValues>();
            if (routeVals == null)
            {
        ...
        {
            SbnRouteValues routeVals = GetSbnRouteValues(context);
