...
context('Content', () => {
    ...
    function refreshContentTree(){
        ...
        cy.sbnContextMenuAction("action-refreshNode").click();
    ...
    function createSimpleMacro(name){
        const insertMacro = new PartialViewMacroBuilder()
            .withName(name)
            .withContent(`@inherits Sbn.Cms.Web.Common.Macros.PartialViewMacroPage
<h1>Acceptance test</h1>`)
    ...
    it('Copy content', () => {
        ...
        cy.deleteAllContent();
        cy.sbnEnsureDocumentTypeNameNotExists(rootDocTypeName);
        cy.sbnEnsureDocumentTypeNameNotExists(childDocTypeName);
        ...
        // Copy node
        cy.sbnTreeItem("content", [nodeName, childNodeName]).rightclick({ force: true });
        cy.sbnContextMenuAction("action-copy").click();
    ...
    it('Move content', () => {
        ...
        // Move node
        cy.sbnTreeItem("content", [nodeName, childNodeName]).rightclick({ force: true });
        cy.sbnContextMenuAction("action-move").click();
        cy.get('.umb-pane [data-element="tree-item-' + anotherNodeName + '"]').click();
        cy.get('.umb-dialog-footer > .btn-primary').click();
    ...
    it('Sort content', () => {
        ...
        // Sort nodes
        cy.sbnTreeItem("content", [nodeName]).rightclick({ force: true });
        cy.sbnContextMenuAction("action-sort").click();
    ...
    it('Rollback content', () => {
        ...
        // Access node
        cy.sbnTreeItem("content", [initialNodeName]).click();

        // Edit header
        cy.get('#headerName').clear();
        cy.sbnEditorHeaderName(nodeName);
    ...
    it('View audit trail', () => {
        ...
        // Access node
        cy.sbnTreeItem("content", [nodeName]).click();
    ...
    it('Preview draft', () => {
        ...
        // Assert
        cy.sbnSuccessNotification({ multiple: true }).should('be.visible');
    ...
    it('Content with contentpicker', () => {
        ...
        cy.deleteAllContent();
        cy.sbnEnsureDocumentTypeNameNotExists(pickerDocTypeName);
        cy.sbnEnsureTemplateNameNotExists(pickerDocTypeName);
        cy.sbnEnsureDocumentTypeNameNotExists(pickedDocTypeName);
        ...
        cy.editTemplate(pickerDocTypeName, `@inherits Sbn.Cms.Web.Common.Views.SbnViewPage<ContentPickerDocType>
        @{
            Layout = null;
        ...
        // Fill out content
        cy.sbnEditorHeaderName('ContentPickerContent');
        ...
        cy.sbnVerifyRenderedViewContent('contentpickercontent', expectedContent, true).should('be.true');
        // clean
        cy.sbnEnsureDocumentTypeNameNotExists(pickerDocTypeName);
        cy.sbnEnsureTemplateNameNotExists(pickerDocTypeName);
        cy.sbnEnsureDocumentTypeNameNotExists(pickedDocTypeName);
    ...
    it('Content with macro in RTE', () => {
        ...
        cy.sbnEnsureMacroNameNotExists(viewMacroName);
        cy.sbnEnsurePartialViewMacroFileNameNotExists(partialFileName);
        cy.sbnEnsureDocumentTypeNameNotExists(viewMacroName);
        cy.sbnEnsureTemplateNameNotExists(viewMacroName);
        cy.deleteAllContent();
        ...
        cy.editTemplate(viewMacroName, `@inherits Sbn.Cms.Web.Common.Views.SbnViewPage
@{
    Layout = null;
}
@{
    if (Model.HasValue("text")){
        ...
        // Enter content
        refreshContentTree();
        cy.sbnTreeItem("content", [viewMacroName]).click();
    ...
    it('Content with macro in grid', () => {
        ...
        cy.sbnEnsureMacroNameNotExists(macroName);
        cy.sbnEnsurePartialViewMacroFileNameNotExists(macroFileName);
        cy.deleteAllContent();
        ...
        cy.editTemplate(name, `@inherits Sbn.Cms.Web.Common.Views.SbnViewPage
        @{
            Layout = null;
        }
