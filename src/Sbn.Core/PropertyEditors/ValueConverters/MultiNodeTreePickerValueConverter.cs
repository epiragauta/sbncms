...
namespace Sbn.Cms.Core.PropertyEditors.ValueConverters
{
    ...
    public class MultiNodeTreePickerValueConverter : PropertyValueConverterBase
    {
        ...
        public override object ConvertIntermediateToObject(IPublishedElement owner, IPublishedPropertyType propertyType, PropertyCacheLevel cacheLevel, object source, bool preview)
        {
            ...
            // TODO: Inject an SbnHelper and create a GetSbnHelper method based on either injected or singleton
            if (_sbnContextAccessor.TryGetSbnContext(out _))
            {
                if (propertyType.EditorAlias.Equals(Constants.PropertyEditors.Aliases.MultiNodeTreePicker))
                {
                    ...
                    if ((propertyType.Alias != null && PropertiesToExclude.InvariantContains(propertyType.Alias)) == false)
                    {
                        ...
                        var objectType = SbnObjectTypes.Unknown;
                        ...
                        foreach (var udi in udis)
                        {
                            ...
                            switch (udi.EntityType)
                            {
                                case Constants.UdiEntityType.Document:
                                    multiNodeTreePickerItem = GetPublishedContent(udi, ref objectType, SbnObjectTypes.Document, id => publishedSnapshot.Content.GetById(guidUdi.Guid));
                                    break;
                                case Constants.UdiEntityType.Media:
                                    multiNodeTreePickerItem = GetPublishedContent(udi, ref objectType, SbnObjectTypes.Media, id => publishedSnapshot.Media.GetById(guidUdi.Guid));
                                    break;
                                case Constants.UdiEntityType.Member:
                                    multiNodeTreePickerItem = GetPublishedContent(udi, ref objectType, SbnObjectTypes.Member, id =>
                                    {
        ...
        private IPublishedContent GetPublishedContent<T>(T nodeId, ref SbnObjectTypes actualType, SbnObjectTypes expectedType, Func<T, IPublishedContent> contentFetcher)
        {
            // is the actual type supported by the content fetcher?
            if (actualType != SbnObjectTypes.Unknown && actualType != expectedType)
            {
                // no, return null
                return null;
            }
