...
namespace Sbn.Cms.Core.Routing
{
    ...
    {
        /// <summary>
        /// Gets the <see cref="SbnRouteResult"/>
        /// </summary>
        public static SbnRouteResult GetRouteResult(this IPublishedRequest publishedRequest)
        {
            if (publishedRequest.IsRedirect())
            {
                return SbnRouteResult.Redirect;
            }

            if (!publishedRequest.HasPublishedContent())
            {
                return SbnRouteResult.NotFound;
            }

            return SbnRouteResult.Success;
        }
