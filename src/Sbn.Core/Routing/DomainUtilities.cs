...
namespace Sbn.Cms.Core.Routing
{
    ...
    public static class DomainUtilities
    {
        ...
        /// <param name="sbnContext">An Sbn context.</param>
        ...
        /// </remarks>
        public static string GetCultureFromDomains(int contentId, string contentPath, Uri current, ISbnContext sbnContext, ISiteDomainMapper siteDomainMapper)
        {
            if (sbnContext == null)
                throw new InvalidOperationException("A current SbnContext is required.");
            ...
            var route = sbnContext.Content.GetRouteById(contentId) ??
                        sbnContext.Content.GetRouteById(true, contentId);
            ...
            var domain = pos == 0
                ? null
                : DomainForNode(sbnContext.Domains, siteDomainMapper, int.Parse(route.Substring(0, pos), CultureInfo.InvariantCulture), current);

            var rootContentId = domain?.ContentId ?? -1;
            var wcDomain = FindWildcardDomainInPath(sbnContext.Domains.GetAll(true), contentPath, rootContentId);

            if (wcDomain != null) return wcDomain.Culture;
            if (domain != null) return domain.Culture;
            return sbnContext.Domains.DefaultCulture;
        }
