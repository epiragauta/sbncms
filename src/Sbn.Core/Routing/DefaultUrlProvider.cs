...
namespace Sbn.Cms.Core.Routing
{
    ...
    public class DefaultUrlProvider : IUrlProvider
    {
        ...
        public DefaultUrlProvider(IOptions<RequestHandlerSettings> requestSettings, ILogger<DefaultUrlProvider> logger, ISiteDomainMapper siteDomainMapper, ISbnContextAccessor sbnContextAccessor, UriUtility uriUtility)
        {
        ...
        public virtual UrlInfo GetUrl(IPublishedContent content, UrlMode mode, string culture, Uri current)
        {
            ...
            var route = sbnContext.Content.GetRouteById(content.Id, culture);

            return GetUrlFromRoute(route, sbnContext, content.Id, current, mode, culture);
        }

        internal UrlInfo GetUrlFromRoute(string route, ISbnContext sbnContext, int id, Uri current, UrlMode mode, string culture)
        {
            ...
            var domainUri = pos == 0
                ? null
                : DomainUtilities.DomainForNode(sbnContext.PublishedSnapshot.Domains, _siteDomainMapper, int.Parse(route.Substring(0, pos), CultureInfo.InvariantCulture), current, culture);
        ...
        /// </summary>
        /// <param name="sbnContextAccessor">The Sbn context.</param>
        ...
        public virtual IEnumerable<UrlInfo> GetOtherUrls(int id, Uri current)
        {
            ...
            while (domainUris == null && n != null) // n is null at root
            {
                ...
                domainUris = n == null ? null : DomainUtilities.DomainsForNode(sbnContext.PublishedSnapshot.Domains, _siteDomainMapper, n.Id, current, excludeDefault: true);
            }
            ...
            foreach (var d in domainUris)
            {
                ...
                var route = sbnContext.Content.GetRouteById(id, culture);
                ...
                uri = _uriUtility.UriFromSbn(uri, _requestSettings);
        ...
        Uri AssembleUrl(DomainAndUri domainUri, string path, Uri current, UrlMode mode)
        {
            ...
            // ignore vdir at that point, UriFromSbn will do it
            ...
            // UriFromSbn will handle vdir
            // meaning it will add vdir into domain URLs too!
            return _uriUtility.UriFromSbn(uri, _requestSettings);
        }
