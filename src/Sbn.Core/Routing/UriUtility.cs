...
namespace Sbn.Cms.Core.Routing
{
    public sealed class UriUtility
    {
        ...
        // maps an internal sbn uri to a public uri
        // ie with virtual directory, .aspx if required...
        public Uri UriFromSbn(Uri uri, RequestHandlerSettings requestConfig)
        {
        ...
        // maps a media sbn uri to a public uri
        // ie with virtual directory - that is all for media
        public Uri MediaUriFromSbn(Uri uri)
        {
            var path = uri.GetSafeAbsolutePath();
            path = ToAbsolute(path);
            return uri.Rewrite(path);
        ...
        // maps a public uri to an internal sbn uri
        // ie no virtual directory, no .aspx, lowercase...
        public Uri UriToSbn(Uri uri)
        {
