...
namespace Sbn.Cms.Core.Strings
{
    ...
    public class DefaultShortStringHelper : IShortStringHelper
    {
        ...
        /// <remarks>Legacy says this was used to "overcome an issue when Sbn is used in IE in an intranet environment" but that issue is not documented.</remarks>
            ...
            CleanStringType caseType, string culture, bool isAcronym)
        {
            ...
            if (isAcronym)
            {
                if ((caseType == CleanStringType.CamelCase && len <= 2 && opos > 0) ||
                        ...
                        (caseType == CleanStringType.SbnCase))
