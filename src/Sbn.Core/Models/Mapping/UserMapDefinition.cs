...
namespace Sbn.Cms.Core.Models.Mapping
{
    public class UserMapDefinition : IMapDefinition
    {
        ...
        // Sbn.Code.MapAll -CreateDate -UpdateDate -DeleteDate
        // Sbn.Code.MapAll -Id -TourData -StartContentIds -StartMediaIds -Language -Username
        // Sbn.Code.MapAll -PasswordQuestion -SessionTimeout -EmailConfirmedDate -InvitedDate
        // Sbn.Code.MapAll -SecurityStamp -Avatar -ProviderUserKey -RawPasswordValue
        // Sbn.Code.MapAll -RawPasswordAnswerValue -Comments -IsApproved -IsLockedOut -LastLoginDate
        // Sbn.Code.MapAll -LastPasswordChangeDate -LastLockoutDate -FailedPasswordAttempts
        // Sbn.Code.MapAll -PasswordConfiguration
        ...
        // Sbn.Code.MapAll -CreateDate -UpdateDate -DeleteDate
        // Sbn.Code.MapAll -TourData -SessionTimeout -EmailConfirmedDate -InvitedDate -SecurityStamp -Avatar
        // Sbn.Code.MapAll -ProviderUserKey -RawPasswordValue -RawPasswordAnswerValue -PasswordQuestion -Comments
        // Sbn.Code.MapAll -IsApproved -IsLockedOut -LastLoginDate -LastPasswordChangeDate -LastLockoutDate
        // Sbn.Code.MapAll -FailedPasswordAttempts
        // Sbn.Code.MapAll -PasswordConfiguration
        ...
        // Sbn.Code.MapAll -ContentStartNode -UserCount -MediaStartNode -Key -Sections
        // Sbn.Code.MapAll -Notifications -Udi -Trashed -AdditionalData -IsSystemUserGroup
        ...
        // Sbn.Code.MapAll -ContentStartNode -MediaStartNode -Sections -Notifications
        // Sbn.Code.MapAll -Udi -Trashed -AdditionalData -IsSystemUserGroup
        ...
        // Sbn.Code.MapAll -Udi -Trashed -AdditionalData -AssignedPermissions
        ...
        // Sbn.Code.MapAll -Trashed -Alias -AssignedPermissions
        ...
        // Sbn.Code.MapAll -ContentStartNode -MediaStartNode -Sections -Notifications -Udi
        // Sbn.Code.MapAll -Trashed -AdditionalData -Users -AssignedPermissions
        private void Map(IUserGroup source, UserGroupDisplay target, MapperContext context)
        {
            ...
            else
            {
                ...
                foreach (var idGroup in allContentPermissions.Keys.InGroupsOf(2000))
                    list.AddRange(_entityService.GetAll(SbnObjectTypes.Document, idGroup.ToArray()));
        ...
        // Sbn.Code.MapAll -Notifications -Udi -Icon -IsCurrentUser -Trashed -ResetPasswordValue
        // Sbn.Code.MapAll -Alias -AdditionalData
        private void Map(IUser source, UserDisplay target, MapperContext context)
        {
            ...
            target.CalculatedStartContentIds = GetStartNodes(source.CalculateContentStartNodeIds(_entityService,_appCaches), SbnObjectTypes.Document, "content","contentRoot", context);
            target.CalculatedStartMediaIds = GetStartNodes(source.CalculateMediaStartNodeIds(_entityService, _appCaches), SbnObjectTypes.Media, "media","mediaRoot", context);
            ...
            target.ParentId = -1;
            target.Path = "-1," + source.Id;
            target.StartContentIds = GetStartNodes(source.StartContentIds.ToArray(), SbnObjectTypes.Document, "content","contentRoot", context);
            target.StartMediaIds = GetStartNodes(source.StartMediaIds.ToArray(), SbnObjectTypes.Media, "media","mediaRoot", context);
        ...
        // Sbn.Code.MapAll -Notifications -IsCurrentUser -Udi -Icon -Trashed -Alias -AdditionalData
        ...
        // Sbn.Code.MapAll -SecondsUntilTimeout
        ...
        private void MapUserGroupBasic(UserGroupBasic target, IEnumerable<string> sourceAllowedSections, int? sourceStartContentId, int? sourceStartMediaId, MapperContext context)
        {
            ...
            if (sourceStartMediaId > 0)
                target.MediaStartNode = context.Map<EntityBasic>(_entityService.Get(sourceStartMediaId.Value, SbnObjectTypes.Media));
            ...
            if (sourceStartContentId > 0)
                target.ContentStartNode = context.Map<EntityBasic>(_entityService.Get(sourceStartContentId.Value, SbnObjectTypes.Document));
        ...
        private IEnumerable<EntityBasic> GetStartNodes(int[] startNodeIds, SbnObjectTypes objectType, string localizedArea,string localizedAlias, MapperContext context)
        {
