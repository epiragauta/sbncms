...
namespace Sbn.Cms.Core.Templates
{
    ...
    public sealed class HtmlLocalLinkParser
    {
        ...
        public HtmlLocalLinkParser(ISbnContextAccessor sbnContextAccessor, IPublishedUrlProvider publishedUrlProvider)
        {
        ...
        public string EnsureInternalLinks(string text, bool preview)
        {
            ...
            {
                throw new InvalidOperationException("Could not parse internal links, there is no current SbnContext");
            }
            ...
            using (sbnContext.ForcedPreview(preview)) // force for URL provider
            {
                return EnsureInternalLinks(text);
            }
        }
