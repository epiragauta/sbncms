...
namespace Sbn.Cms.Core.Composing
{
    ...
    {
        private readonly HashSet<string> _sbnAssemblies;
        ...
        {
            _sbnAssemblies = new HashSet<string>(targetAssemblies, StringComparer.Ordinal);
        ...
        public IEnumerable<Assembly> ResolveAssemblies()
        {
            ...
            // for Sbn dependencies/transitive dependencies
            foreach(var dir in assemblyLocations)
            {
                foreach(var dll in Directory.EnumerateFiles(dir, "*.dll"))
                {
                    ...
                    if (assemblyName != null)
                    {
                        ...
                        // don't include this item if it's Sbn Core
                        if (Constants.Composing.SbnCoreAssemblyNames.Any(x=>assemblyName.FullName.StartsWith(x) || assemblyName.Name.EndsWith(".Views")))
                            continue;
            ...
            foreach (var item in assemblies)
            {
                ...
                if (classification == Classification.ReferencesSbn || classification == Classification.IsSbn)
                {
        ...
        private Classification Resolve(Assembly assembly)
        {
            ...
            {
                // if its part of the filter it doesn't reference sbn
                classification = Classification.DoesNotReferenceSbn;
            }
            else if (_sbnAssemblies.Contains(assembly.GetName().Name))
            {
                classification = Classification.IsSbn;
            }
            else
            {
                classification = Classification.DoesNotReferenceSbn;
                foreach (var reference in GetReferences(assembly))
                {
                    ...
                    if (referenceClassification == Classification.IsSbn || referenceClassification == Classification.ReferencesSbn)
                    {
                        classification = Classification.ReferencesSbn;
                        break;
                    }
                }
            }
