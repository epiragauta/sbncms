            ...
            bool onlyConcreteClasses)
        {
            ...
            while (stack.Count > 0)
            {
                ...
                if (assembly != attributeType.Assembly || attributeAssemblyIsCandidate)
                {
                    ...
                    {
                        _logger.LogError(ex, "Could not query types on {Assembly} assembly, this is most likely due to this assembly not being compatible with the current Sbn version", assembly);
                        continue;
                    }
