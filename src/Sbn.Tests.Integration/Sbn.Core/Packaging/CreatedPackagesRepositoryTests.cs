...
namespace Sbn.Cms.Tests.Integration.Sbn.Core.Packaging
{
    [TestFixture]
    [SbnTest(Database = SbnTestOptions.Database.NewSchemaPerFixture)]
    public class CreatedPackagesRepositoryTests : SbnIntegrationTest
    {
        ...
        public void Export_Zip()
        {
            ...
            using (ZipArchive zipArchive = PackageMigrationResource.GetPackageDataManifest(packageZipStream, out XDocument packageXml))
            {
                ...
                Assert.Multiple(() =>
                {
                    ...
                    Assert.AreEqual(
                        $"<MediaItems><MediaSet><testImage id=\"{m1.Id}\" key=\"{m1.Key}\" parentID=\"-1\" level=\"1\" creatorID=\"-1\" sortOrder=\"0\" createDate=\"{m1.CreateDate.ToString("s")}\" updateDate=\"{m1.UpdateDate.ToString("s")}\" nodeName=\"Test File\" urlName=\"test-file\" path=\"{m1.Path}\" isDoc=\"\" nodeType=\"{mt.Id}\" nodeTypeAlias=\"testImage\" writerName=\"\" writerID=\"0\" udi=\"{m1.GetUdi()}\" mediaFilePath=\"/media/test-file.txt\"><sbnFile><![CDATA[/media/test-file.txt]]></sbnFile><sbnBytes><![CDATA[100]]></sbnBytes><sbnExtension><![CDATA[png]]></sbnExtension></testImage></MediaSet></MediaItems>",
        ...
        public void Export_Xml()
        {
            ...
            using (var packageXmlStream = File.OpenRead(packageXmlPath))
            {
                ...
                Assert.Multiple(() =>
                {
                    ...
                    Assert.AreEqual($"<Templates><Template><Name>Text page</Name><Key>{template.Key}</Key><Alias>textPage</Alias><Design><![CDATA[@using Sbn.Cms.Web.Common.PublishedModels;{Environment.NewLine}@inherits Sbn.Cms.Web.Common.Views.SbnViewPage{Environment.NewLine}@{{{Environment.NewLine}\tLayout = null;{Environment.NewLine}}}]]></Design></Template></Templates>", xml.Element("umbPackage").Element("Templates").ToString(SaveOptions.DisableFormatting));
