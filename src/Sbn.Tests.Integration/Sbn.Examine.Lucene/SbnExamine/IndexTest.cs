...
namespace Sbn.Cms.Tests.Integration.Sbn.Examine.Lucene.SbnExamine
{
    ...
    public class IndexTest : ExamineBaseTest
    {
        ...
        {
            using (GetSynchronousContentIndex(false, out SbnContentIndex index, out _, out _, 999))
            {
        ...
        {
            using (GetSynchronousContentIndex(false, out SbnContentIndex index, out _, out ContentValueSetBuilder contentValueSetBuilder, null))
            {
                index.CreateIndex();
                ...
                Assert.IsTrue(result.Values.ContainsKey($"{SbnExamineFieldNames.RawFieldPrefix}grid"));
                Assert.AreEqual(json, result[$"{SbnExamineFieldNames.RawFieldPrefix}grid"]);
            }
        }
        ...
        {            
            ...
            using (GetSynchronousContentIndex(false, out SbnContentIndex index, out ContentIndexPopulator contentRebuilder, out _, null))
            {
        ...
        {
            using (GetSynchronousContentIndex(true, out SbnContentIndex index, out ContentIndexPopulator contentRebuilder, out _, null))
            {
                //create the whole thing
                contentRebuilder.Populate(index);
        ...
        public void GivenMediaUnderNonIndexableParent_WhenMediaMovedUnderIndexableParent_ThenItIsIncludedInTheIndex()
        {
            ...
            using (GetSynchronousContentIndex(false, out SbnContentIndex index, out ContentIndexPopulator contentRebuilder, out _, 1116))
            {
        ...
        public void GivenMediaUnderIndexableParent_WhenMediaMovedUnderNonIndexableParent_ThenItIsRemovedFromTheIndex()
        {
            ...
            using (GetSynchronousContentIndex(false, out SbnContentIndex index, out ContentIndexPopulator contentRebuilder, out _, 2222))
            {
                var searcher = index.Searcher;
