...
namespace Sbn.Cms.Tests.Integration.Testing
{
    public abstract class BaseTestDatabase
    {
        ...
        protected ISbnDatabaseFactory _databaseFactory;
        protected IList<TestDbMeta> _testDatabases;


        protected SbnDatabase.CommandInfo[] _cachedDatabaseInitCommands = new SbnDatabase.CommandInfo[0];
        ...
        private void RebuildSchema(IDbCommand command, TestDbMeta meta)
        {
            ...
            foreach (SbnDatabase.CommandInfo dbCommand in _cachedDatabaseInitCommands)
            {
                ...
                foreach (SbnDatabase.ParameterInfo parameterInfo in dbCommand.Parameters)
                {
        ...
        private void RebuildSchemaFirstTime(TestDbMeta meta)
        {
            ...
            using (var database = (SbnDatabase)_databaseFactory.CreateDatabase())
            {
                database.LogCommands = true;

                using (NPoco.ITransaction transaction = database.GetTransaction())
                {
                    var schemaCreator = new DatabaseSchemaCreator(database, _loggerFactory.CreateLogger<DatabaseSchemaCreator>(), _loggerFactory, new SbnVersion(), Mock.Of<IEventAggregator>());
        ...
        protected static void AddParameter(IDbCommand cmd, SbnDatabase.ParameterInfo parameterInfo)
        {
