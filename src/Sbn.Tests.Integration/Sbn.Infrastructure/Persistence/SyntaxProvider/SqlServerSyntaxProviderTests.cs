...
namespace Sbn.Cms.Tests.Integration.Sbn.Infrastructure.Persistence.SyntaxProvider
{
    [TestFixture]
    public class SqlServerSyntaxProviderTests : SbnIntegrationTest
    {
        private ISqlContext SqlContext => GetRequiredService<ISbnDatabaseFactory>().SqlContext;
...
FROM [cmsContentNu]
INNER JOIN [sbnNode]
ON [cmsContentNu].[nodeId] = [sbnNode].[id]
WHERE (([sbnNode].[nodeObjectType] = @0))) x)".Replace(Environment.NewLine, " ").Replace("\n", " ").Replace("\r", " "),
