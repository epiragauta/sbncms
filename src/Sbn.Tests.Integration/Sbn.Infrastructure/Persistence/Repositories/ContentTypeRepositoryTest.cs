...
namespace Sbn.Cms.Tests.Integration.Sbn.Infrastructure.Persistence.Repositories
{
    ...
    public class ContentTypeRepositoryTest : SbnIntegrationTest
    {
        ...
        private ISbnMapper Mapper => GetRequiredService<ISbnMapper>();
        ...
        public void Can_Move()
        {
            ...
            using (IScope scope = provider.CreateScope())
            {
                ...
                global::Sbn.Cms.Core.Events.MoveEventInfo<IContentType>[] result = repository.Move(contentType, container1).ToArray();
            ...
            using (IScope scope = provider.CreateScope())
            {
                ...
                int count =
                    scope.Database.ExecuteScalar<int>(
                        "SELECT COUNT(*) FROM sbnNode WHERE nodeObjectType = @NodeObjectType",
