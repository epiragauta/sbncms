...
namespace Sbn.Cms.Tests.Integration.Sbn.Infrastructure.Persistence.Repositories
{
    ...
    public class RelationRepositoryTest : SbnIntegrationTest
    {
        ...
        public void Get_Paged_Parent_Entities_By_Child_Id()
        {
            ...
            using (IScope scope = ScopeProvider.CreateScope())
            {
                ...
                parents.AddRange(repository.GetPagedParentEntitiesByChildId(createdMedia[0].Id, 0, 100, out totalRecords, SbnObjectTypes.Document.GetGuid()));
                Assert.AreEqual(3, totalRecords);

                parents.AddRange(repository.GetPagedParentEntitiesByChildId(createdMedia[0].Id, 0, 100, out totalRecords, SbnObjectTypes.Member.GetGuid()));
                Assert.AreEqual(3, totalRecords);

                parents.AddRange(repository.GetPagedParentEntitiesByChildId(createdMedia[0].Id, 0, 100, out totalRecords, SbnObjectTypes.Media.GetGuid()));
        ...
        public void Get_Paged_Child_Entities_By_Parent_Id()
        {
            ...
            using (IScope scope = ScopeProvider.CreateScope())
            {
                ...
                // only of a certain type
                parents.AddRange(repository.GetPagedChildEntitiesByParentId(createdContent[0].Id, 0, 100, out totalRecords, SbnObjectTypes.Media.GetGuid()));
                Assert.AreEqual(3, totalRecords);

                parents.AddRange(repository.GetPagedChildEntitiesByParentId(createdMembers[0].Id, 0, 100, out totalRecords, SbnObjectTypes.Media.GetGuid()));
                Assert.AreEqual(3, totalRecords);

                parents.AddRange(repository.GetPagedChildEntitiesByParentId(createdContent[0].Id, 0, 100, out totalRecords, SbnObjectTypes.Member.GetGuid()));
                Assert.AreEqual(0, totalRecords);
            }
        }
