...
namespace Sbn.Cms.Web.BackOffice.Filters
{
    ...
    internal sealed class DataTypeValidateAttribute : TypeFilterAttribute
    {
        ...
        private class DataTypeValidateFilter : IActionFilter
        {
            ...
            public DataTypeValidateFilter(IDataTypeService dataTypeService, PropertyEditorCollection propertyEditorCollection, ISbnMapper sbnMapper)
            {
            ...
            public void OnActionExecuting(ActionExecutingContext context)
            {
                ...
                if (!_propertyEditorCollection.TryGet(dataType.EditorAlias, out var propertyEditor))
                {
                    ...
                    context.Result = new SbnProblemResult(message, HttpStatusCode.NotFound);
                    return;
                }
                ...
                switch (dataType.Action)
                {
                    case ContentSaveAction.Save:
                        ...
                        _sbnMapper.Map(dataType, persisted);
                        break;

                    case ContentSaveAction.SaveNew:
                        // create the persisted model from mapping the saved model
                        persisted = _sbnMapper.Map<IDataType>(dataType);
                        ((DataType)persisted).ResetIdentity();
                        break;

                    default:
                        context.Result = new SbnProblemResult($"Data type action {dataType.Action} was not found.", HttpStatusCode.NotFound);
                        return;
                }
