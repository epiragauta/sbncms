...
namespace Sbn.Cms.Web.BackOffice.Mapping
{
    ...
    internal class ContentMapDefinition : IMapDefinition
    {
        ...
        // Sbn.Code.MapAll -AllowPreview -Errors -PersistedContent
        private void Map(IContent source, ContentItemDisplay target, MapperContext context)
        {
            ...
            target.ContentTypeName = _localizedTextService.SbnDictionaryTranslate(_cultureDictionary, source.ContentType.Name);
        ...
        // Sbn.Code.MapAll -Segment -Language -DisplayName
        ...
        private IEnumerable<string> GetActions(IContent source, IContent parent, MapperContext context)
        {
            ...
            // with the ISbnContextAccessor. In the meantime, if used outside of a web app this will throw a null
            // reference exception :(
        ...
        private UrlInfo[] GetUrls(IContent source)
        {
            ...
            {
                return new[] { UrlInfo.Message("Cannot generate URLs without a current Sbn Context") };
            }

            // NOTE: unfortunately we're not async, we'll use .Result and hope this won't cause a deadlock anywhere for now
            var urls = source.GetContentUrlsAsync(_publishedRouter, sbnContext, _localizationService, _localizedTextService, _contentService, _variationContextAccessor, _loggerFactory.CreateLogger<IContent>(), _uriUtility, _publishedUrlProvider)
        ...
        private IDictionary<string, string> GetAllowedTemplates(IContent source)
        {
            ...
            return contentType.AllowedTemplates
                ...
                .ToDictionary(t => t.Alias, t => _localizedTextService.SbnDictionaryTranslate(_cultureDictionary, t.Name));
        }
