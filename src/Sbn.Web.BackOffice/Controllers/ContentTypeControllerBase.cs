...
namespace Sbn.Cms.Web.BackOffice.Controllers
{
        ...
        where TContentType : class, IContentTypeComposition
    {
            ...
            ILocalizedTextService localizedTextService)
        {
            ...
            SbnMapper = sbnMapper ?? throw new ArgumentNullException(nameof(sbnMapper));
        ...
        public ISbnMapper SbnMapper { get; }
            ...
            bool isElement)
        {
            ...
            return availableCompositions.Results
                .Select(x => new Tuple<EntityBasic, bool>(SbnMapper.Map<IContentTypeComposition, EntityBasic>(x.Composition), x.Allowed))
                .Select(x =>
                {
        ...
        private IEnumerable<EntityContainer> GetEntityContainers(IContentTypeComposition contentType, SbnObjectTypes type)
        {
            if (contentType == null)
            {
                return null;
        ...
        /// <returns></returns>
        protected ActionResult<IEnumerable<EntityBasic>> PerformGetWhereCompositionIsUsedInContentTypes(int contentTypeId, SbnObjectTypes type)
        {
            var id = 0;
            ...
            return composedOf
                .Select(SbnMapper.Map<IContentTypeComposition, EntityBasic>)
            ...
            where TPropertyType : PropertyTypeBasic
        {
            ...
            if (ctId > 0)
            {
                ...
                try
                {
                    SbnMapper.Map(contentTypeSave, ct);
                }
                catch (Exception ex)
                {
            ...
            else
            {
                ...
                {
                    //This mapping will cause a lot of content type validation to occur which we need to deal with
                    newCt = SbnMapper.Map<TContentType>(contentTypeSave);
                }
                catch (Exception ex)
                {
                    var responseEx = CreateInvalidCompositionResponseException<TContentTypeDisplay, TContentTypeSave, TPropertyType>(ex, contentTypeSave, ct, ctId);
            ...
            where TContentTypeDisplay : ContentTypeCompositionDisplay
        {
            ...
            if (validateAttempt == false)
            {
                ...
                var display = SbnMapper.Map<TContentTypeDisplay>(composition);
                //map the 'save' data on top
                display = SbnMapper.Map(contentTypeSave, display);
            ...
            where TContentTypeSave : ContentTypeSave
        {
            ...
            if (ctId > 0)
            {
                ...
                forDisplay = SbnMapper.Map<TContentTypeDisplay>(ct);
                //map the 'save' data on top
                forDisplay = SbnMapper.Map(contentTypeSave, forDisplay);
            }
            else
            {
                //map the 'save' data to display
                forDisplay = SbnMapper.Map<TContentTypeDisplay>(contentTypeSave);
            }
