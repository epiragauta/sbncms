...
{
    /// <summary>
    /// The API controller used for getting entity objects, basic name, icon, id representation of sbn objects that are based on CMSNode
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    public class EntityController : SbnAuthorizedJsonController
    {
        ...
        /// <summary>
        /// Returns an Sbn alias given a string
        /// </summary>
        ...
        /// <returns></returns>
        [HttpGet]
        public IEnumerable<EntityBasic> Search(string query, SbnEntityTypes type, string searchFrom = null, Guid? dataTypeKey = null)
        {
            ...
            //TODO: This uses the internal SbnTreeSearcher, this instead should delgate to the ISearchableTree implementation for the type
        ...
        /// <param name="type"></param>
        /// <returns></returns>
        public IConvertToActionResult GetPath(int id, SbnEntityTypes type)
        {
        ...
        /// <param name="type"></param>
        /// <returns></returns>
        public IConvertToActionResult GetPath(Guid id, SbnEntityTypes type)
        {
        ...
        /// <param name="type"></param>
        /// <returns></returns>
        public IActionResult GetPath(Udi id, SbnEntityTypes type)
        {
        ...
        public IActionResult GetUrl(Udi id, string culture = "*")
        {
            ...
            switch (id.EntityType)
            {
                case Constants.UdiEntityType.Document:
                    entityType = SbnEntityTypes.Document;
                    break;
                case Constants.UdiEntityType.Media:
                    entityType = SbnEntityTypes.Media;
                    break;
                case Constants.UdiEntityType.Member:
                    entityType = SbnEntityTypes.Member;
                    break;
                default:
                    return NotFound();
            }
        ...
        /// </remarks>
        public IActionResult GetUrl(int id, SbnEntityTypes type, string culture = null)
        {
            culture = culture ?? ClientCulture();

            var returnUrl = string.Empty;

            if (type == SbnEntityTypes.Document)
            {
        ...
        /// <param name="type"></param>
        /// <returns></returns>
        public ActionResult<EntityBasic> GetByQuery(string query, int nodeContextId, SbnEntityTypes type)
        {
        ...
        {
            return SbnXPathPathSyntaxParser.ParseXPathQuery(
        ...
        /// <param name="type"></param>
        /// <returns></returns>
        public ActionResult<EntityBasic> GetById(int id, SbnEntityTypes type)
        {
        ...
        /// <param name="type"></param>
        /// <returns></returns>
        public ActionResult<EntityBasic> GetById(Guid id, SbnEntityTypes type)
        {
        ...
        /// <param name="type"></param>
        /// <returns></returns>
        public ActionResult<EntityBasic> GetById(Udi id, SbnEntityTypes type)
        {
            var guidUdi = id as GuidUdi;
            if (guidUdi != null)
            {
        ...
        [HttpGet]
        [HttpPost]
        public ActionResult<IEnumerable<EntityBasic>> GetByIds([FromJsonPath]int[] ids, [FromQuery]SbnEntityTypes type)
        {
            if (ids == null)
            {
                return NotFound();
        ...
        [HttpGet]
        [HttpPost]
        public ActionResult<IEnumerable<EntityBasic>> GetByIds([FromJsonPath]Guid[] ids, [FromQuery]SbnEntityTypes type)
        {
            if (ids == null)
            {
                return NotFound();
        ...
        [HttpGet]
        [HttpPost]
        public ActionResult<IEnumerable<EntityBasic>> GetByIds([FromJsonPath]Udi[] ids, [FromQuery]SbnEntityTypes type)
        {
            if (ids == null)
            {
                return NotFound();
        ...
        public IEnumerable<EntityBasic> GetChildren(int id, SbnEntityTypes type, Guid? dataTypeKey = null)
        {
            ...
            if (objectType.HasValue)
            {
                ...
                if (id == Constants.System.Root && startNodes.Length > 0 && startNodes.Contains(Constants.System.Root) == false && !ignoreUserStartNodes)
                {
                    ...
                    if (nodes.Length == 0)
                        return Enumerable.Empty<EntityBasic>();
                    var pr = new List<EntityBasic>(nodes.Select(_sbnMapper.Map<EntityBasic>));
                    return pr;
                }
                ...
                return _entityService.GetChildren(id, objectType.Value)
                    .WhereNotNull()
                    .Select(_sbnMapper.Map<EntityBasic>);
            }
            //now we need to convert the unknown ones
            switch (type)
            {
                case SbnEntityTypes.Language:
                case SbnEntityTypes.User:
                case SbnEntityTypes.Macro:
                default:
            ...
            Guid? dataTypeKey = null)
        {
            ...
            if (objectType.HasValue)
            {
                ...
                if (id == Constants.System.Root && startNodes.Length > 0 && startNodes.Contains(Constants.System.Root) == false && !ignoreUserStartNodes)
                {
                    ...
                    {
                        Items = nodes.Select(_sbnMapper.Map<EntityBasic>)
                    };
                    return pr;
                }
                ...
                entities = _entityService.GetPagedChildren(id, objectType.Value, pageNumber - 1, pageSize, out totalRecords,
                    filter.IsNullOrWhiteSpace()
                        ? null
                        : _sqlContext.Query<ISbnEntity>().Where(x => x.Name.Contains(filter)),
                ...
                var pagedResult = new PagedResult<EntityBasic>(totalRecords, pageNumber, pageSize)
                {
                    ...
                    {
                        var target = _sbnMapper.Map<IEntitySlim, EntityBasic>(source, context =>
                        {
            ...
            switch (type)
            {
                case SbnEntityTypes.PropertyType:
                case SbnEntityTypes.PropertyGroup:
                case SbnEntityTypes.Language:
                case SbnEntityTypes.User:
                case SbnEntityTypes.Macro:
                default:
            ...
            Guid? dataTypeKey = null)
        {
            ...
            if (objectType.HasValue)
            {
                IEnumerable<ISbnEntity> entities;
                long totalRecords;
        ...
        public IEnumerable<EntityBasic> GetAncestors(int id, SbnEntityTypes type, [ModelBinder(typeof(HttpQueryStringModelBinder))]FormCollection queryStrings)
        {
            return GetResultForAncestors(id, type, queryStrings);
        }

        public ActionResult<IEnumerable<EntityBasic>> GetAncestors(Guid id, SbnEntityTypes type, [ModelBinder(typeof(HttpQueryStringModelBinder))]FormCollection queryStrings)
        {
        ...
        /// <returns></returns>
        private IEnumerable<SearchResultEntity> ExamineSearch(string query, SbnEntityTypes entityType, string searchFrom = null, bool ignoreUserStartNodes = false)
        {
            var culture = ClientCulture();
            return _treeSearcher.ExamineSearch(query, entityType, 200, 0, out _, culture, searchFrom, ignoreUserStartNodes);
        }

        private IEnumerable<EntityBasic> GetResultForChildren(int id, SbnEntityTypes entityType)
        {
        ...
        private IEnumerable<EntityBasic> GetResultForAncestors(int id, SbnEntityTypes entityType, FormCollection queryStrings = null)
        {
            var objectType = ConvertToObjectType(entityType);
            if (objectType.HasValue)
            {
        ...
        private IEnumerable<EntityBasic> GetResultForKeys(Guid[] keys, SbnEntityTypes entityType)
        {
            if (keys.Length == 0)
                return Enumerable.Empty<EntityBasic>();
        ...
        private IEnumerable<EntityBasic> GetResultForIds(int[] ids, SbnEntityTypes entityType)
        {
            if (ids.Length == 0)
                return Enumerable.Empty<EntityBasic>();
        ...
        private ActionResult<EntityBasic> GetResultForKey(Guid key, SbnEntityTypes entityType)
        {
            var objectType = ConvertToObjectType(entityType);
            if (objectType.HasValue)
            {
                ...
                return _sbnMapper.Map<IEntitySlim, EntityBasic>(found);
            }
            //now we need to convert the unknown ones
            switch (entityType)
            {
        ...
        private ActionResult<EntityBasic> GetResultForId(int id, SbnEntityTypes entityType)
        {
            var objectType = ConvertToObjectType(entityType);
            if (objectType.HasValue)
            {
        ...
        private static SbnObjectTypes? ConvertToObjectType(SbnEntityTypes entityType)
        {
            switch (entityType)
            {
                case SbnEntityTypes.Document:
                ...
                case SbnEntityTypes.MemberType:
                    return SbnObjectTypes.MemberType;
                case SbnEntityTypes.MemberGroup:
                    return SbnObjectTypes.MemberGroup;
                case SbnEntityTypes.MediaType:
                    return SbnObjectTypes.MediaType;
                case SbnEntityTypes.DocumentType:
                    return SbnObjectTypes.DocumentType;
                case SbnEntityTypes.Member:
                    return SbnObjectTypes.Member;
                case SbnEntityTypes.DataType:
                    return SbnObjectTypes.DataType;
                default:
                    //There is no SbnEntity conversion (things like Macros, Users, etc...)
                    return null;
            }
        }
        ...
        /// <returns></returns>
        public IEnumerable<EntityBasic> GetAll(SbnEntityTypes type, string postFilter)
        {
        ...
        /// <returns></returns>
        private IEnumerable<EntityBasic> GetResultForAll(SbnEntityTypes entityType, string postFilter = null)
        {
            var objectType = ConvertToObjectType(entityType);
            if (objectType.HasValue)
            {
            ...
            switch (entityType)
            {
                ...
                case SbnEntityTypes.PropertyType:
                    ...
                    return _sbnMapper.MapEnumerable<IPropertyType, EntityBasic>(filteredPropertyTypes);
                ...
                case SbnEntityTypes.PropertyGroup:
                    ...
                    return _sbnMapper.MapEnumerable<PropertyGroup, EntityBasic>(filteredpropertyGroups);

                case SbnEntityTypes.User:

                    var users = _userService.GetAll(0, int.MaxValue, out _);
                    var filteredUsers = ExecutePostFilter(users, postFilter);
                    return _sbnMapper.MapEnumerable<IUser, EntityBasic>(filteredUsers);

                case SbnEntityTypes.Stylesheet:
                ...
                case SbnEntityTypes.PartialView:
                ...
                case SbnEntityTypes.DictionaryItem:
        ...
        {
            culture = culture ?? ClientCulture();
            return _sbnMapper.Map<EntityBasic>(entity, context => { context.SetCulture(culture); });
        }
        ...
        private IEnumerable<EntityBasic> GetAllDictionaryItems()
        {
            ...
            {
                var item = _sbnMapper.Map<IDictionaryItem, EntityBasic>(dictionaryItem);
                list.Add(item);
        ...
        private void GetChildItemsForList(IDictionaryItem dictionaryItem, ICollection<EntityBasic> list)
        {
            ...
            {
                var item = _sbnMapper.Map<IDictionaryItem, EntityBasic>(childItem);
                list.Add(item);
