...
namespace Sbn.Cms.Web.BackOffice.Controllers
{
    ...
    public class MacroRenderingController : SbnAuthorizedJsonController
    {
        ...
        public ActionResult<IEnumerable<MacroParameter>> GetMacroParameters(int macroId)
        {
            ...
            return new ActionResult<IEnumerable<MacroParameter>>(_sbnMapper.Map<IEnumerable<MacroParameter>>(macro).OrderBy(x => x.SortOrder));
        }
        ...
        private async Task<IActionResult> GetMacroResultAsHtml(string macroAlias, int pageId, IDictionary<string, object> macroParams)
        {
            ...
            var publishedContent = sbnContext.Content.GetById(true, pageId);
            ...
            var culture = DomainUtilities.GetCultureFromDomains(publishedContent.Id, publishedContent.Path, null, sbnContext, _siteDomainHelper);
            ...
            using (sbnContext.ForcedPreview(true))
            {
                //need to create a specific content result formatted as HTML since this controller has been configured
                //with only json formatters.
