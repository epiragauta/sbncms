...
namespace Sbn.Cms.Web.BackOffice.Controllers
{
    ...
    public class ContentController : ContentControllerBase
    {
        ...
        private ActionResult<IEnumerable<AssignedUserGroupPermissions>> GetDetailedPermissions(IContent content, IEnumerable<IUserGroup> allUserGroups)
        {
            ...
            var defaultPermissionsByGroup = _sbnMapper.MapEnumerable<IUserGroup, AssignedUserGroupPermissions>(allUserGroups);
        ...
        private ContentItemDisplay CleanContentItemDisplay(ContentItemDisplay display)
        {
            ...
            display.ContentTypeName = _localizedTextService.SbnDictionaryTranslate(CultureDictionary, display.ContentTypeName);
            // if your user type doesn't have access to the Settings section it would not get this property mapped
            if (display.DocumentType != null)
                display.DocumentType.Name = _localizedTextService.SbnDictionaryTranslate(CultureDictionary, display.DocumentType.Name);
        ...
        public ActionResult<ContentItemDisplay> GetEmptyBlueprint(int blueprintId, int parentId)
        {
            ...
            var mapped = _sbnMapper.Map<ContentItemDisplay>(blueprint);
                ...
                string cultureName = "") // TODO: it's not a NAME it's the ISO CODE
        {
            ...
            pagedResult.Items = children.Select(content =>
                _sbnMapper.Map<IContent, ContentItemBasic<ContentPropertyBasic>>(content,
                    context =>
                    {
        ...
        public ActionResult<DomainSave> PostSaveLanguageAndDomains(DomainSave model)
        {
            ...
            if (language != null)
            {
                ...
                {
                    wildcard = new SbnDomain("*" + model.NodeId)
                    {
            ...
            foreach (var domainModel in model.Domains.Where(m => string.IsNullOrWhiteSpace(m.Name) == false))
            {
                ...
                {
                    // yet there is a race condition here...
                    var newDomain = new SbnDomain(name)
                    {
        ...
        private void AddMessageForPublishStatus(IReadOnlyCollection<PublishResult> statuses, INotificationModel display, string[] successfulCultures = null)
        {
            ...
            foreach (var status in statusGroup)
            {
                switch (status.Key)
                {
                    case PublishResultType.SuccessPublishAlready:
                        {
                            // TODO: Here we should have messaging for when there are release dates specified like https://github.com/sbn/Sbn-CMS/pull/3507
                            ...
                            // http://github.com/sbn/Sbn-CMS/commit/9b3de7b655e07c612c824699b48a533c0448131a
        ...
        {
            var display = _sbnMapper.Map<ContentItemDisplay>(content, contextOptions);
