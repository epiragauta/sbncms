    ...
    public class PublicAccessController : BackOfficeNotificationsController
    {
        ...
        public ActionResult<PublicAccess> GetPublicAccess(int contentId)
        {
            ...
            var nodes = _entityService
                .GetAll(SbnObjectTypes.Document, entry.LoginNodeId, entry.NoAccessNodeId)
                .ToDictionary(x => x.Id);
            ...
            MemberDisplay[] members = usernames
                ...
                .Select(_sbnMapper.Map<MemberDisplay>)
                .ToArray();
            ...
            MemberGroupDisplay[] groups = entry.Rules
                ...
                .Select(rule => allGroups.TryGetValue(rule.RuleValue, out SbnIdentityRole memberRole) ? memberRole : null)
                .Where(x => x != null)
                .Select(_sbnMapper.Map<MemberGroupDisplay>)
                .ToArray();
            ...
            return new PublicAccess
            {
                Members = members,
                Groups = groups,
                LoginPage = loginPageEntity != null ? _sbnMapper.Map<EntityBasic>(loginPageEntity) : null,
                ErrorPage = errorPageEntity != null ? _sbnMapper.Map<EntityBasic>(errorPageEntity) : null
            };
        }
