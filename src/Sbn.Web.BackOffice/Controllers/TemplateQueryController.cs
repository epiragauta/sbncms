...
namespace Sbn.Cms.Web.BackOffice.Controllers
{
    ...
    public class TemplateQueryController : SbnAuthorizedJsonController
    {
        ...
        public QueryResultModel PostTemplateQuery(QueryModel model)
        {
            ...
            if (model == null)
            {
                ...
                queryExpression.Append("Sbn.ContentAtRoot().FirstOrDefault().Children()");
            }
            else
            {
        ...
        private IEnumerable<IPublishedContent> PostTemplateValue(QueryModel model, StringBuilder queryExpression)
        {
            ...
            if (model.Source != null && model.Source.Id > 0)
            {
                ...
                if (sourceDocument == null)
                    queryExpression.AppendFormat("Sbn.Content({0})", model.Source.Id);
                else
                    queryExpression.AppendFormat("Sbn.Content(Guid.Parse(\"{0}\"))", sourceDocument.Key);
            }
            else
            {
                sourceDocument = _publishedContentQuery.ContentAtRoot().FirstOrDefault();
                queryExpression.Append("Sbn.ContentAtRoot().FirstOrDefault()");
            }
