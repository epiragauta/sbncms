...
namespace Sbn.Cms.Web.BackOffice.Controllers
{
    ...
    public class DashboardController : SbnApiController
    {
        ...
        public async Task<JObject> GetRemoteDashboardContent(string section, string baseUrl = "https://dashboard.sbn.com/")
        {
            var user = _backOfficeSecurityAccessor.BackOfficeSecurity.CurrentUser;
            var allowedSections = string.Join(",", user.AllowedSections);
            var language = user.Language;
            var version = _sbnVersion.SemanticVersion.ToSemanticStringWithoutBuild();
            ...
            var url = string.Format("{0}{1}?section={2}&allowed={3}&lang={4}&version={5}&admin={6}",
                ...
                version,
                isAdmin);
            var key = "sbn-dynamic-dashboard-" + language + allowedSections.Replace(",", "-") + section;
        ...
        public async Task<IActionResult> GetRemoteDashboardCss(string section, string baseUrl = "https://dashboard.sbn.org/")
        {
            var url = string.Format(baseUrl + "css/dashboard.css?section={0}", section);
            var key = "sbn-dynamic-dashboard-css-" + section;
        ...
        public async Task<IActionResult> GetRemoteXml(string site, string url)
        {
            ...
            // Which was used to grab data from our.sbn.com, sbn.com or sbn.tv
            // for certain dashboards or the help drawer
            var urlPrefix = string.Empty;
            switch (site.ToUpper())
            {
                case "TV":
                    urlPrefix = "https://sbn.tv/";
                    break;

                case "OUR":
                    urlPrefix = "https://our.sbn.com/";
                    break;

                case "COM":
                    urlPrefix = "https://sbn.com/";
                    break;
            ...
            var key = $"sbn-XML-feed-{site}-{url.ToCleanString(_shortStringHelper, CleanStringType.UrlSegment)}";
