    ...
    public abstract class ContentTreeControllerBase : TreeController, ITreeNodeController
    {
        ...
        public ActionResult<TreeNode> GetTreeNode([FromRoute] string id, [ModelBinder(typeof(HttpQueryStringModelBinder))] FormCollection queryStrings)
        {
            ...
            var entity = asGuid == Guid.Empty
                    ? _entityService.Get(asInt, SbnObjectType)
                    : _entityService.Get(asGuid, SbnObjectType);
            if (entity == null)
            {
                return NotFound();
            }
        ...
        /// <summary>
        /// Returns a <see cref="TreeNode"/> for the <see cref="ISbnEntity"/> and
        ...
        protected virtual ActionResult<TreeNodeCollection> PerformGetTreeNodes(string id, FormCollection queryStrings)
        {
            ...
            if (id == rootIdString && hasAccessToRoot == false)
            {
                ...
                {
                    var topNodes = _entityService.GetAll(SbnObjectType, topNodeIds.ToArray());
        ...
        private int GetTopNodeId(ISbnEntity entity)
        {
            int id;
        ...
        protected abstract SbnObjectTypes SbnObjectType { get; }
        ...
        protected virtual ActionResult<IEnumerable<IEntitySlim>> GetChildEntities(string id, FormCollection queryStrings)
        {
            ...
            if (!ignoreUserStartNodes && entityId == Constants.System.Root && UserStartNodes.Contains(Constants.System.Root) == false)
            {
                result = UserStartNodes.Length > 0
                    ? _entityService.GetAll(SbnObjectType, UserStartNodes).ToArray()
        ...
        private IEnumerable<IEntitySlim> GetChildrenFromEntityService(int entityId)
            => _entityService.GetChildren(entityId, SbnObjectType).ToList();
        ...
        /// <param name="queryStrings"></param>
        /// <returns></returns>
        protected bool HasPathAccess(ISbnEntity entity, FormCollection queryStrings)
        {
            if (entity == null)
                return false;
        ...
        protected bool ShouldRenderChildrenOfContainer(IEntitySlim e)
        {
            ...
            {
                var startNodes = _entityService.GetAll(SbnObjectType, UserStartNodes);
        ...
        internal IEnumerable<MenuItem> GetAllowedUserMenuItemsForNode(ISbnEntity dd)
        {
        ...
        /// <returns></returns>
        internal bool CanUserAccessNode(ISbnEntity doc, IEnumerable<MenuItem> allowedUserOptions, string culture)
        {
        ...
        internal IEntitySlim GetEntityFromId(string id)
        {
            return _entityCache.GetOrAdd(id, s =>
            {
                ...
                {
                    entity = _entityService.Get(idGuid, SbnObjectType);
                }
                else if (int.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out var idInt))
                {
                    entity = _entityService.Get(idInt, SbnObjectType);
                }
                else if (UdiParser.TryParse(s, out var idUdi))
                {
                    var guidUdi = idUdi as GuidUdi;
                    entity = guidUdi != null ? _entityService.Get(guidUdi.Guid, SbnObjectType) : null;
                }
                else
                {
                    entity = null;
