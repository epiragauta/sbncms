...
namespace Sbn.Cms.Core.Security
{
    ...
    /// </summary>
    public class MemberUserStore : SbnUserStore<MemberIdentityUser, SbnIdentityRole>, IMemberUserStore
    {
        ...
        public override Task<IdentityResult> UpdateAsync(MemberIdentityUser user, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                if (!int.TryParse(user.Id, NumberStyles.Integer, CultureInfo.InvariantCulture, out var asInt))
                {
                    ...
                    throw new InvalidOperationException("The user id must be an integer to work with the Sbn");
                }
                using IScope scope = _scopeProvider.CreateScope(autoComplete: true);
        ...
        /// <inheritdoc/>
        protected override Task<SbnIdentityRole> FindRoleAsync(string roleName, CancellationToken cancellationToken)
        {
            if (string.IsNullOrWhiteSpace(roleName))
            {
                throw new ArgumentNullException(nameof(roleName));
            ...
            if (group == null)
            {
                return Task.FromResult((SbnIdentityRole)null);
            }

            return Task.FromResult(new SbnIdentityRole(group.Name)
            {
