    ...
    public class NewInstallStep : InstallSetupStep<UserModel>
    {
        ...
        public override async Task<InstallSetupResult> ExecuteAsync(UserModel user)
        {
            ...
            if (user.SubscribeToNewsLetter)
            {
                ...
                try
                {
                    var response = httpClient.PostAsync("https://shop.sbn.com/base/Ecom/SubmitEmail/installer.aspx", content).Result;
                }
        ...
        private InstallState GetInstallState()
        {
            ...
            var databaseSettings = _connectionStrings.CurrentValue.SbnConnectionString;
            ...
            var sbnInstalled = canConnect ? _databaseBuilder.IsSbnInstalled() : false;
            if (sbnInstalled)
            {
                installState = (installState | InstallState.SbnInstalled) & ~InstallState.Unknown;
            }

            var hasNonDefaultUser = sbnInstalled ? _databaseBuilder.HasSomeNonDefaultUser() : false;
        ...
        private bool ShowView()
        {
            ...
            return installState.HasFlag(InstallState.Unknown)
                || !installState.HasFlag(InstallState.SbnInstalled);
        }
