...
namespace Sbn.Cms.Core.Configuration
{
    public class JsonConfigManipulator : IConfigManipulator
    {
        ...
        public string SbnConnectionPath { get; } = $"ConnectionStrings:{ Cms.Core.Constants.System.SbnConnectionName}";
        public void RemoveConnectionString()
        {
            var provider = GetJsonConfigurationProvider(SbnConnectionPath);

            var json = GetJson(provider);

            RemoveJsonKey(json, SbnConnectionPath);
        ...
        private JToken GetConnectionItem(string connectionString, string providerName)
        {
            ...
            writer.WriteStartObject();
            writer.WritePropertyName(Cms.Core.Constants.System.SbnConnectionName);
