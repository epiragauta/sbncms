...
using Sbn.Cms.Infrastructure.Logging.Serilog;
...
namespace Sbn.Extensions
{
    public static class LoggerConfigExtensions
    {
            ...
            IConfiguration configuration)
        {
            ...
            //This is not optimal, but seems to be the only way if we do not make an Serilog.Sink.SbnFile sink all the way.
            var sbnFileConfiguration = new SbnFileConfiguration(configuration);

            logConfig.WriteTo.SbnFile(
                path : sbnFileConfiguration.GetPath(loggingConfiguration.LogDirectory),
                fileSizeLimitBytes: sbnFileConfiguration.FileSizeLimitBytes,
                restrictedToMinimumLevel: sbnFileConfiguration.RestrictedToMinimumLevel,
                rollingInterval: sbnFileConfiguration.RollingInterval,
                flushToDiskInterval: sbnFileConfiguration.FlushToDiskInterval,
                rollOnFileSizeLimit: sbnFileConfiguration.RollOnFileSizeLimit,
                retainedFileCountLimit: sbnFileConfiguration.RetainedFileCountLimit
            );
            ...
            LogEventLevel minimumLevel = LogEventLevel.Verbose)
        {
            ...
            logConfig.WriteTo.File(Path.Combine(hostingEnvironment.MapPathContentRoot(Cms.Core.Constants.SystemDirectories.LogFiles),  $"SbnTraceLog.{Environment.MachineName}..txt"),
                shared: true,
        ...
        /// </remarks>
        public static LoggerConfiguration SbnFile(this LoggerSinkConfiguration configuration,
            string path,
            ...
            ILoggingConfiguration loggingConfiguration, LogEventLevel minimumLevel = LogEventLevel.Verbose, int? retainedFileCount = null)
        {
            ...
            logConfig.WriteTo.File(new CompactJsonFormatter(),
                Path.Combine(hostingEnvironment.MapPathContentRoot(Cms.Core.Constants.SystemDirectories.LogFiles) ,$"SbnTraceLog.{Environment.MachineName}..json"),
                shared: true,
