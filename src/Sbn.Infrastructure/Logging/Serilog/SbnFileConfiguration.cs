...
namespace Sbn.Cms.Infrastructure.Logging.Serilog
{
    public class SbnFileConfiguration
    {
        public SbnFileConfiguration(IConfiguration configuration)
        {
            ...
            var sbnFileAppSettings = appSettings.GetChildren().LastOrDefault(x => x.GetValue<string>("Name") == "SbnFile");

            if (sbnFileAppSettings is not null)
            {
                var args = sbnFileAppSettings.GetSection("Args");
        ...
        {
            return Path.Combine(logDirectory, $"SbnTraceLog.{Environment.MachineName}..json");
        }
    }
}
