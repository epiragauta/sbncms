...
using Sbn.Core.Collections;
using Sbn.Cms.Core.Cache;
using Sbn.Cms.Core.Events;
using Sbn.Cms.Core.IO;
using Sbn.Cms.Infrastructure.Persistence;
using CoreDebugSettings = Sbn.Cms.Core.Configuration.Models.CoreDebugSettings;
...
namespace Sbn.Cms.Core.Scoping
{
    ...
    internal class Scope : IScope
    {
        ...
        private ISbnDatabase _database;
        ...
        public ISbnDatabase Database
        {
            get
            {
                ...
                if (_database != null)
                {
                    ...
                    // In our case within Sbn, I don't think this ever occurs, all locks are requested
                    ...
                    // SbnDatabase instance directly and ensure it's called when OnExecutingCommand
                ...
                if (ParentScope != null)
                {
                    ISbnDatabase database = ParentScope.Database;
        ...
        public ISbnDatabase DatabaseOrNull
        {
            get
            {
                EnsureNotDisposed();
        ...
        // true if Sbn.CoreDebugSettings.LogUncompletedScope appSetting is set to "true"
        ...
        private void EagerReadLockInner(ISbnDatabase db, Guid instanceId, TimeSpan? timeout, params int[] lockIds)
        {
            if (ParentScope is not null)
            {
        ...
        /// <param name="timeout">Optional database timeout in milliseconds.</param>
        /// <param name="lockIds">Array of lock object identifiers.</param>
        private void EagerWriteLockInner(ISbnDatabase db, Guid instanceId, TimeSpan? timeout, params int[] lockIds)
        {
            if (ParentScope is not null)
            {
                // If we have a parent we delegate lock creation to parent.
        ...
        private void LockInner(ISbnDatabase db, Guid instanceId, ref Dictionary<Guid, Dictionary<int, int>> locks, ref HashSet<int> locksSet,
            Action<ISbnDatabase, int> obtainLock, Action<ISbnDatabase, int, TimeSpan> obtainLockTimeout, TimeSpan? timeout,
            params int[] lockIds)
        {
            lock (_dictionaryLocker)
            {
        ...
        private void ObtainReadLock(ISbnDatabase db, int lockId)
        {
        ...
        private void ObtainTimeoutReadLock(ISbnDatabase db, int lockId, TimeSpan timeout)
        {
        ...
        /// </summary>
        /// <param name="lockId">Lock object identifier to lock.</param>
        private void ObtainWriteLock(ISbnDatabase db, int lockId)
        {
        ...
        /// <param name="lockId">Lock object identifier to lock.</param>
        /// <param name="timeout">TimeSpan specifying the timout period.</param>
        private void ObtainTimeoutWriteLock(ISbnDatabase db, int lockId, TimeSpan timeout)
        {
