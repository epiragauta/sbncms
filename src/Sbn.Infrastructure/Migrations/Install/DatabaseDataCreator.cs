...
namespace Sbn.Cms.Infrastructure.Migrations.Install
{
    ...
    internal class DatabaseDataCreator
    {
        ...
        public DatabaseDataCreator(IDatabase database, ILogger<DatabaseDataCreator> logger, ISbnVersion sbnVersion)
        {
            _database = database;
            _logger = logger;
        ...
        /// <summary>
        /// Initialize the base data creation by inserting the data foundation for sbn
        /// specific to a table
        /// </summary>
        ...
        private void CreateNodeData()
        {
            ...
            _database.Insert(Cms.Core.Constants.DatabaseSchema.Tables.Node, "id", false, new NodeDto { NodeId = -1, Trashed = false, ParentId = -1, UserId = -1, Level = 0, Path = "-1", SortOrder = 0, UniqueId = new Guid("916724a5-173d-4619-b97e-b9de133dd6f5"), Text = "SYSTEM DATA: sbn master root", NodeObjectType = Cms.Core.Constants.ObjectTypes.SystemRoot, CreateDate = DateTime.Now });
        ...
        private void CreateDataTypeData()
        {
            ...
            // TODO: Check which of the DataTypeIds below doesn't exist in sbnNode, which results in a foreign key constraint errors.
            ...
            _database.Insert(Cms.Core.Constants.DatabaseSchema.Tables.DataType, "pk", false, new DataTypeDto { NodeId = -87, EditorAlias = Cms.Core.Constants.PropertyEditors.Aliases.TinyMce, DbType = "Ntext",
                Configuration = "{\"value\":\",code,undo,redo,cut,copy,mcepasteword,stylepicker,bold,italic,bullist,numlist,outdent,indent,mcelink,unlink,mceinsertanchor,mceimage,sbnmacro,mceinserttable,sbnembed,mcecharmap,|1|1,2,3,|0|500,400|1049,|true|\"}" });
            ...
            InsertDataTypeDto(Cms.Core.Constants.DataTypes.LabelString, Cms.Core.Constants.PropertyEditors.Aliases.Label, "Nvarchar", "{\"sbnDataValueType\":\"STRING\"}");
            InsertDataTypeDto(Cms.Core.Constants.DataTypes.LabelInt, Cms.Core.Constants.PropertyEditors.Aliases.Label, "Integer", "{\"sbnDataValueType\":\"INT\"}");
            InsertDataTypeDto(Cms.Core.Constants.DataTypes.LabelBigint, Cms.Core.Constants.PropertyEditors.Aliases.Label, "Nvarchar", "{\"sbnDataValueType\":\"BIGINT\"}");
            InsertDataTypeDto(Cms.Core.Constants.DataTypes.LabelDateTime, Cms.Core.Constants.PropertyEditors.Aliases.Label, "Date", "{\"sbnDataValueType\":\"DATETIME\"}");
            InsertDataTypeDto(Cms.Core.Constants.DataTypes.LabelDecimal, Cms.Core.Constants.PropertyEditors.Aliases.Label, "Decimal", "{\"sbnDataValueType\":\"DECIMAL\"}");
            InsertDataTypeDto(Cms.Core.Constants.DataTypes.LabelTime, Cms.Core.Constants.PropertyEditors.Aliases.Label, "Date", "{\"sbnDataValueType\":\"TIME\"}");
            ...
            _database.Insert(Cms.Core.Constants.DatabaseSchema.Tables.DataType, "pk", false, new DataTypeDto { NodeId = -41, EditorAlias = "Sbn.DateTime", DbType = "Date", Configuration = "{\"format\":\"YYYY-MM-DD\"}" });
        ...
        {
            // on install, initialize the sbn migration plan with the final state
