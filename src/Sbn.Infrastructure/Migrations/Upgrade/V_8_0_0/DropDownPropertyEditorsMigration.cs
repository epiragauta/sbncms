...
namespace Sbn.Cms.Infrastructure.Migrations.Upgrade.V_8_0_0
{
    public class DropDownPropertyEditorsMigration : PropertyEditorsMigrationBase
    {
        ...
        protected override void Migrate()
        {
            ...
            // bypassing the services, then we need to rebuild the cache entirely, including the sbnContentNu table
            if (refreshCache)
        ...
        private bool Migrate(IEnumerable<DataTypeDto> dataTypes)
        {
            ...
            foreach (var dataType in dataTypes)
            {
                ...
                {
                    case string ea when ea.InvariantEquals("Sbn.DropDown"):
                        UpdateDataType(dataType, config, false);
                        break;
                    case string ea when ea.InvariantEquals("Sbn.DropdownlistPublishingKeys"):
                        UpdateDataType(dataType, config, false);
                        break;
                    case string ea when ea.InvariantEquals("Sbn.DropDownMultiple"):
                        UpdateDataType(dataType, config, true);
                        break;
                    case string ea when ea.InvariantEquals("Sbn.DropdownlistMultiplePublishKeys"):
                        UpdateDataType(dataType, config, true);
                        break;
                }
