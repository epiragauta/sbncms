...
namespace Sbn.Cms.Infrastructure.Migrations.Upgrade.V_8_0_0
{
    public class AddLockObjects : MigrationBase
    {
        ...
        internal static void EnsureLockObject(ISbnDatabase db, int id, string name)
        {
            // not if it already exists
            var exists = db.Exists<LockDto>(id);
            if (exists) return;

            // be safe: delete old sbnNode lock objects if any
            db.Execute($"DELETE FROM sbnNode WHERE id={id};");

            // then create sbnLock object
            db.Execute($"INSERT sbnLock (id, name, value) VALUES ({id}, '{name}', 1);");
        }
    }
}
