...
namespace Sbn.Cms.Infrastructure.Search
{
    ...
    /// <summary>
    /// Used for internal Sbn implementations of <see cref="ISearchableTree"/>
    /// </summary>
    public class SbnTreeSearcher
    {
        private readonly ILocalizationService _languageService;
        private readonly IEntityService _entityService;
        private readonly ISbnMapper _mapper;
        private readonly ISqlContext _sqlContext;
        private readonly IBackOfficeExamineSearcher _backOfficeExamineSearcher;
        private readonly IPublishedUrlProvider _publishedUrlProvider;


        public SbnTreeSearcher(
            ...
            long pageIndex, out long totalFound, string culture = null, string searchFrom = null, bool ignoreUserStartNodes = false)
        {
            ...
            switch (entityType)
            {
                ...
                default:
                    throw new NotSupportedException("The " + typeof(SbnTreeSearcher) + " currently does not support searching against object type " + entityType);
            }
        }
        ...
        /// <returns></returns>
        public IEnumerable<SearchResultEntity> EntitySearch(SbnObjectTypes objectType, string query, int pageSize, long pageIndex, out long totalFound, string searchFrom = null)
        {
            ...
            var results = _entityService.GetPagedDescendants(objectType, pageIndex, pageSize, out totalFound,
                filter: _sqlContext.Query<ISbnEntity>().Where(x => x.Name.Contains(query) || x.Key == g));
        ...
        private IEnumerable<SearchResultEntity> MemberFromSearchResults(IEnumerable<ISearchResult> results)
        {
            ...
            foreach (var result in results)
            {
                ...
                if (result.Values.ContainsKey(SbnExamineFieldNames.NodeKeyFieldName) && result.Values[SbnExamineFieldNames.NodeKeyFieldName] != null)
                {
                    if (Guid.TryParse(result.Values[SbnExamineFieldNames.NodeKeyFieldName], out var key))
                    {
                        m.Key = key;
                    }
                }
        ...
        private IEnumerable<SearchResultEntity> ContentFromSearchResults(IEnumerable<ISearchResult> results, string culture = null)
        {
            ...
            foreach (var result in results)
            {
                ...
                if (int.TryParse(entity.Id.ToString(),NumberStyles.Integer, CultureInfo.InvariantCulture, out var intId))
                {
                    ...
                    if (result.Values.TryGetValue(SbnExamineFieldNames.VariesByCultureFieldName, out var varies) && varies == "y")
                    {
