    ...
    public class MultiUrlPickerValueEditor : DataValueEditor, IDataValueReference
    {
        ...
        public override object ToEditor(IProperty property, string culture = null, string segment = null)
        {
            ...
            try
            {
                ...
                {
                    entities.AddRange(
                        _entityService.GetAll(SbnObjectTypes.Document, documentLinks.Select(link => link.Udi.Guid).ToArray())
                    );
                }
                ...
                {
                    entities.AddRange(
                        _entityService.GetAll(SbnObjectTypes.Media, mediaLinks.Select(link => link.Udi.Guid).ToArray())
                    );
                }
                ...
                foreach (var dto in links)
                {
                    ...
                    if (dto.Udi != null)
                    {
                        ISbnEntity entity = entities.Find(e => e.Key == dto.Udi.Guid);
                        if (entity == null)
                        {
                            continue;
                        }
        ...
        public IEnumerable<SbnEntityReference> GetReferences(object value)
        {
            ...
            foreach (var link in links)
            {
                ...
                {
                    yield return new SbnEntityReference(link.Udi);
                }
