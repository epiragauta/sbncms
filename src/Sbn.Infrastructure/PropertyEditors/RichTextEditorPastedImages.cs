    ...
    public sealed class RichTextEditorPastedImages
    {
        ...
        public string FindAndPersistPastedTempImages(string html, Guid mediaParentFolder, int userId, IImageUrlGenerator imageUrlGenerator)
        {
            ...
            foreach (var img in tmpImages)
            {
                ...
                _sbnContextAccessor.TryGetSbnContext(out var sbnContext);
                var mediaTyped = sbnContext?.Media.GetById(udi.Guid);
                if (mediaTyped == null)
                    throw new PanicException($"Could not find media by id {udi.Guid} or there was no SbnContext available.");
