...
namespace Sbn.Cms.Infrastructure.Packaging
{
    public class PackageDataInstallation
    {
        ...
        private EntityContainer CreateContentTypeChildFolder(string folderName, Guid folderKey, ISbnEntity current)
        {
        ...
        private EntityContainer CreateDataTypeChildFolder(string folderName, Guid folderKey, ISbnEntity current)
        {
            var children = _entityService.GetChildren(current.Id).ToArray();
            var found = children.Any(x => x.Name.InvariantEquals(folderName) ||x.Key.Equals(folderKey));
            if (found)
