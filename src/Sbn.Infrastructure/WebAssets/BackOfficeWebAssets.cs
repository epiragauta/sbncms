...
namespace Sbn.Cms.Infrastructure.WebAssets
{
    ...
    {
        public const string SbnPreviewJsBundleName = "sbn-preview-js";
        public const string SbnPreviewCssBundleName = "sbn-preview-css";
        public const string SbnCssBundleName = "sbn-backoffice-css";
        public const string SbnInitCssBundleName = "sbn-backoffice-init-css";
        public const string SbnCoreJsBundleName = "sbn-backoffice-js";
        public const string SbnExtensionsJsBundleName = "sbn-backoffice-extensions-js";
        public const string SbnNonOptimizedPackageJsBundleName = "sbn-backoffice-non-optimized-js";
        public const string SbnNonOptimizedPackageCssBundleName = "sbn-backoffice-non-optimized-css";
        public const string SbnTinyMceJsBundleName = "sbn-tinymce-js";
        public const string SbnUpgradeCssBundleName = "sbn-authorize-upgrade-css";
        ...
        public void CreateBundles()
        {
            ...
            _runtimeMinifier.CreateCssBundle(SbnInitCssBundleName,
                BundlingOptions.NotOptimizedAndComposite,
                FormatPaths(
                    "assets/css/sbn.min.css",
                    "lib/bootstrap-social/bootstrap-social.css",
                    "lib/font-awesome/css/font-awesome.min.css"));

            _runtimeMinifier.CreateCssBundle(SbnUpgradeCssBundleName,
                BundlingOptions.NotOptimizedAndComposite,
                FormatPaths(
                    "assets/css/sbn.min.css",
                    "lib/bootstrap-social/bootstrap-social.css",
            ...
            _runtimeMinifier.CreateCssBundle(SbnPreviewCssBundleName,
                BundlingOptions.NotOptimizedAndComposite,
                FormatPaths("assets/css/canvasdesigner.min.css"));

            _runtimeMinifier.CreateJsBundle(SbnPreviewJsBundleName,
                BundlingOptions.NotOptimizedAndComposite,
                FormatPaths(GetScriptsForPreview()));

            _runtimeMinifier.CreateJsBundle(SbnTinyMceJsBundleName,
                BundlingOptions.NotOptimizedAndComposite,
                FormatPaths(GetScriptsForTinyMce()));

            _runtimeMinifier.CreateJsBundle(SbnCoreJsBundleName,
                BundlingOptions.NotOptimizedAndComposite,
            ...
            RegisterPackageBundlesForNoneOption(_parser.CombinedManifest.Scripts, SbnNonOptimizedPackageJsBundleName);
            ...
            // Create a single non-optimized (no file processing) bundle for all manifests declaring None as a bundle option
            RegisterPackageBundlesForNoneOption(_parser.CombinedManifest.Stylesheets, SbnNonOptimizedPackageCssBundleName);
        }
        ...
        {
            var sbnPath = _globalSettings.GetSbnMvcArea(_hostingEnvironment);

            return assets
                .Where(x => x.IsNullOrWhiteSpace() == false)
                .Select(x => !x.StartsWith("/") && Uri.IsWellFormedUriString(x, UriKind.Relative)
                    // most declarations with be made relative to the /sbn folder, so things
                    // like lib/blah/blah.js so we need to turn them into absolutes here
                    ? sbnPath.EnsureStartsWith('/').TrimEnd("/") + x.EnsureStartsWith('/')
                    : x).ToArray();
        }
