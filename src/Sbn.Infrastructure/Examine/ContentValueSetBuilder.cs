        ...
        private IEnumerable<ValueSet> GetValueSetsEnumerable(IContent[] content, Dictionary<int, IProfile> creatorIds, Dictionary<int, IProfile> writerIds)
        {
            ...
            foreach (var c in content)
            {
                ...
                var values = new Dictionary<string, IEnumerable<object>>
                {
                    ...
                    {SbnExamineFieldNames.PublishedFieldName, new object[] {c.Published ? "y" : "n"}},   //Always add invariant published value
                    {"id", new object[] {c.Id}},
                    {SbnExamineFieldNames.NodeKeyFieldName, new object[] {c.Key}},
                    ...
                    {SbnExamineFieldNames.NodeNameFieldName, (PublishedValuesOnly               //Always add invariant nodeName
                        ? c.PublishName?.Yield()
                    ...
                    {SbnExamineFieldNames.VariesByCultureFieldName, new object[] {"n"}},
                };

                if (isVariant)
                {
                    values[SbnExamineFieldNames.VariesByCultureFieldName] = new object[] { "y" };
                    ...
                    foreach (var culture in c.AvailableCultures)
                    {
                        ...
                        values[$"{SbnExamineFieldNames.PublishedFieldName}_{lowerCulture}"] = (c.IsCulturePublished(culture) ? "y" : "n").Yield<object>();
