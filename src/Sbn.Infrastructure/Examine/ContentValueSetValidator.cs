    ...
    public class ContentValueSetValidator : ValueSetValidator, IContentValueSetValidator
    {
        ...
        public bool ValidatePath(string path, string category)
        {
            ...
            if (ParentId.HasValue && ParentId.Value > 0)
            {
                ...
                // because we need to remove anything that doesn't pass by parent Id in the cases that sbn data is moved to an illegal parent.
        ...
        public override ValueSetValidationResult Validate(ValueSet valueSet)
        {
            ...
            {
                if (!valueSet.Values.TryGetValue(SbnExamineFieldNames.PublishedFieldName, out var published))
                {
                    return ValueSetValidationResult.Failed;
                }
                ...
                if (valueSet.Values.TryGetValue(SbnExamineFieldNames.VariesByCultureFieldName, out var variesByCulture)
                    && variesByCulture.Count > 0 && variesByCulture[0].Equals("y"))
                {
                    ...
                    foreach (var publishField in valueSet.Values.Where(x => x.Key.StartsWith($"{SbnExamineFieldNames.PublishedFieldName}_")).ToList())
                    {
            ...
            // We cannot return FAILED here because we need the value set to get into the indexer and then deal with it from there
            // because we need to remove anything that doesn't pass by protected content in the cases that sbn data is moved to an illegal parent.
