    ...
    public class UserGroupRepository : EntityRepositoryBase<int, IUserGroup>, IUserGroupRepository
    {
        ...
        public IUserGroup Get(string alias)
        {
            try
            {
                ...
                {
                    var groupId = Database.ExecuteScalar<int?>("SELECT id FROM sbnUserGroup WHERE userGroupAlias=@alias", new { alias });
        ...
        public IEnumerable<IUserGroup> GetGroupsAssignedToSection(string sectionAlias)
        {
            ...
            //SELECT *
            //FROM [sbnUserGroup]
            //LEFT JOIN [sbnUserGroup2App]
            //ON [sbnUserGroup].[id] = [sbnUserGroup2App].[user]
            //WHERE sbnUserGroup.id IN (SELECT sbnUserGroup.id
            //    FROM [sbnUserGroup]
            //    LEFT JOIN [sbnUserGroup2App]
            //    ON [sbnUserGroup].[id] = [sbnUserGroup2App].[user]
            //    WHERE sbnUserGroup2App.app = 'content')

            var sql = GetBaseQuery(QueryType.Many);
            var innerSql = GetBaseQuery(QueryType.Ids);
            innerSql.Where("sbnUserGroup2App.app = " + SqlSyntax.GetQuotedValue(sectionAlias));
            sql.Where($"sbnUserGroup.id IN ({innerSql.SQL})");
            AppendGroupBy(sql);
        ...
        protected override IEnumerable<string> GetDeleteClauses()
        {
            var list = new List<string>
                           {
                               "DELETE FROM sbnUser2UserGroup WHERE userGroupId = @id",
                               "DELETE FROM sbnUserGroup2App WHERE userGroupId = @id",
                               "DELETE FROM sbnUserGroup2NodePermission WHERE userGroupId = @id",
                               "DELETE FROM sbnUserGroup WHERE id = @id"
                           };
            return list;
        }
