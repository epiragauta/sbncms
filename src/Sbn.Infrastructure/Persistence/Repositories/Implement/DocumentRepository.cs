...
namespace Sbn.Cms.Infrastructure.Persistence.Repositories.Implement
{
    ...
    public class DocumentRepository : ContentRepositoryBase<int, IContent, DocumentRepository>, IDocumentRepository
    {
        ...
        // (or go further and have it on ISbnEntity): "IsReadOnly" and if that is true we throw
        ...
        public override void DeleteVersion(int versionId)
        {
            ...
            var template = SqlContext.Templates.Get("Sbn.Core.DocumentRepository.GetVersion", tsql =>
        ...
        public override void DeleteVersions(int nodeId, DateTime versionDate)
        {
            ...
            var template = SqlContext.Templates.Get("Sbn.Core.DocumentRepository.GetVersions", tsql =>
               tsql.Select<ContentVersionDto>()
                    .From<ContentVersionDto>()
                    .InnerJoin<DocumentVersionDto>()
                    .On<ContentVersionDto, DocumentVersionDto>((c, d) => c.Id == d.Id)
        ...
        protected override void PersistUpdatedItem(IContent entity)
        {
            ...
            if (!isMoving)
            {
                ...
                if (entity.ContentType.VariesByCulture())
                {
                    ...
                    // 'Sbn.Core.Persistence.FaultHandling.RetryDbConnection' to type 'System.Data.SqlClient.SqlConnection'.
        ...
        protected override void PersistDeletedItem(IContent entity)
        {
            ...
            Database.Execute(SqlContext.SqlSyntax.GetDeleteSubquery("sbnAccessRule", "accessId", subQuery));
        ...
        {
            var template = SqlContext.Templates.Get("Sbn.Core.DocumentRepository.GetSqlForHasScheduling", tsql => tsql
                .SelectCount()
        ...
        private SqlTemplate SqlEnsureVariantNamesAreUnique => SqlContext.Templates.Get("Sbn.Core.DomainRepository.EnsureVariantNamesAreUnique", tsql => tsql
