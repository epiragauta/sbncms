    ...
    internal class RedirectUrlRepository : EntityRepositoryBase<Guid, IRedirectUrl>, IRedirectUrlRepository
    {
        ...
        protected override Sql<ISqlContext> GetBaseQuery(bool isCount)
        {
            ...
            if (isCount)
                sql.Select(@"COUNT(*)
FROM sbnRedirectUrl
JOIN sbnNode ON sbnRedirectUrl.contentKey=sbnNode.uniqueID");
            else
                sql.Select(@"sbnRedirectUrl.*, sbnNode.id AS contentId
FROM sbnRedirectUrl
JOIN sbnNode ON sbnRedirectUrl.contentKey=sbnNode.uniqueID");
            return sql;
        }
        ...
        protected override IEnumerable<string> GetDeleteClauses()
        {
            var list = new List<string>
                {
                    "DELETE FROM sbnRedirectUrl WHERE id = @id"
                };
            return list;
        }
        ...
        {
            Database.Execute("DELETE FROM sbnRedirectUrl");
        }

        public void DeleteContentUrls(Guid contentKey)
        {
            Database.Execute("DELETE FROM sbnRedirectUrl WHERE contentKey=@contentKey", new { contentKey });
        }
        ...
        {
            var sql = GetBaseQuery(false)
                .Where(string.Format("{0}.{1} LIKE @path", SqlSyntax.GetQuotedTableName("sbnNode"), SqlSyntax.GetQuotedColumnName("path")), new { path = "%," + rootContentId + ",%" })
                .OrderByDescending<RedirectUrlDto>(x => x.CreateDateUtc);
            var result = Database.Page<RedirectUrlDto>(pageIndex + 1, pageSize, sql);
            total = Convert.ToInt32(result.TotalItems);
        ...
        {
            var sql = GetBaseQuery(false)
                .Where(string.Format("{0}.{1} LIKE @url", SqlSyntax.GetQuotedTableName("sbnRedirectUrl"), SqlSyntax.GetQuotedColumnName("Url")), new { url = "%" + searchTerm.Trim().ToLowerInvariant() + "%" })
                .OrderByDescending<RedirectUrlDto>(x => x.CreateDateUtc);
            var result = Database.Page<RedirectUrlDto>(pageIndex + 1, pageSize, sql);
            total = Convert.ToInt32(result.TotalItems);
