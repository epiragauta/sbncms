...
namespace Sbn.Cms.Infrastructure.Persistence.Repositories.Implement
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>Limited to objects that have a corresponding node (in sbnNode table).</para>
    ...
    internal class EntityRepository : RepositoryBase, IEntityRepository
    {
        ...
        public IEnumerable<IEntitySlim> GetPagedResultsByQuery(IQuery<ISbnEntity> query, Guid objectType, long pageIndex, int pageSize, out long totalRecords,
        ...
        public IEnumerable<IEntitySlim> GetPagedResultsByQuery(IQuery<ISbnEntity> query, Guid[] objectTypes, long pageIndex, int pageSize, out long totalRecords,
            IQuery<ISbnEntity> filter, Ordering ordering, Action<Sql<ISqlContext>> sqlCustomization = null)
        {
            ...
            var translator = new SqlTranslator<ISbnEntity>(sql, query);
        ...
        public IEnumerable<IEntitySlim> GetByQuery(IQuery<ISbnEntity> query)
        {
            var sqlClause = GetBase(false, false, false, null);
            var translator = new SqlTranslator<ISbnEntity>(sqlClause, query);
        ...
        public IEnumerable<IEntitySlim> GetByQuery(IQuery<ISbnEntity> query, Guid objectType)
        {
            var isContent = objectType == Cms.Core.Constants.ObjectTypes.Document || objectType == Cms.Core.Constants.ObjectTypes.DocumentBlueprint;
            var isMedia = objectType == Cms.Core.Constants.ObjectTypes.Media;
            var isMember = objectType == Cms.Core.Constants.ObjectTypes.Member;
        ...
        public SbnObjectTypes GetObjectType(int id)
        {
            var sql = Sql().Select<NodeDto>(x => x.NodeObjectType).From<NodeDto>().Where<NodeDto>(x => x.NodeId == id);
            return ObjectTypes.GetSbnObjectType(Database.ExecuteScalar<Guid>(sql));
        }

        public SbnObjectTypes GetObjectType(Guid key)
        {
