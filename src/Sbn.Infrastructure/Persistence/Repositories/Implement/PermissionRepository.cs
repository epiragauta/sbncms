...
namespace Sbn.Cms.Infrastructure.Persistence.Repositories.Implement
{
        ...
        where TEntity : class, IEntity
    {
        ...
        public void ReplacePermissions(int groupId, IEnumerable<char> permissions, params int[] entityIds)
        {
            ...
            var sql = "DELETE FROM sbnUserGroup2NodePermission WHERE userGroupId = @groupId AND nodeId in (@nodeIds)";
        ...
        public void AssignPermission(int groupId, char permission, params int[] entityIds)
        {
            ...
            var sql = "DELETE FROM sbnUserGroup2NodePermission WHERE userGroupId = @groupId AND permission=@permission AND nodeId in (@entityIds)";
            db.Execute(sql,
            new
            {
                groupId,
        ...
        public void AssignEntityPermission(TEntity entity, char permission, IEnumerable<int> groupIds)
        {
            ...
            const string sql = "DELETE FROM sbnUserGroup2NodePermission WHERE nodeId = @nodeId AND permission = @permission AND userGroupId in (@groupIds)";
            db.Execute(sql,
                new
                {
                    nodeId = entity.Id,
        ...
        public void ReplaceEntityPermissions(EntityPermissionSet permissionSet)
        {
            ...
            const string sql = "DELETE FROM sbnUserGroup2NodePermission WHERE nodeId = @nodeId";
