...
INNER JOIN sbnNode ON cmsContentType.nodeId = sbnNode.id
WHERE cmsContentType." + SqlSyntax.GetQuotedColumnName("alias") + @"= @alias
AND sbnNode.nodeObjectType = @objectType",
            ...
            int sortOrder =
                Database.ExecuteScalar<int>("SELECT COUNT(*) FROM sbnNode WHERE parentID = @ParentId AND nodeObjectType = @NodeObjectType",
                                                      new { ParentId = entity.ParentId, NodeObjectType = NodeObjectTypeId });

            //Create the (base) node data - sbnNode
...
AND sbnNode.id <> @id",
            ...
            if (entity.IsPropertyDirty("PropertyGroups"))
            {
                ...
                if (groupsToDelete.Length > 0)
                {
                    ...
                    // see http://issues.sbn.org/issue/U4-8663
        ...
        /// <summary>
        /// Re-normalizes the edited value in the sbnDocumentCultureVariation and sbnDocument table when variations are changed
        /// </summary>
        ...
        private void RenormalizeDocumentEditedFlags(IReadOnlyCollection<int> propertyTypeIds, IReadOnlyCollection<int> contentTypeIds = null)
        {
            ...
            //lookup all matching rows in sbnDocumentCultureVariation
            ...
            //Now bulk update the sbnDocument table
