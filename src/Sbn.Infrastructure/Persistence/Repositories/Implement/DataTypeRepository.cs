...
namespace Sbn.Cms.Infrastructure.Persistence.Repositories.Implement
{
    ...
    internal class DataTypeRepository : EntityRepositoryBase<int, IDataType>, IDataTypeRepository
    {
        ...
        protected override IEnumerable<IDataType> PerformGetAll(params int[] ids)
        {
            ...
            if (ids.Any())
            {
                dataTypeSql.Where("sbnNode.id in (@ids)", new { ids });
            }
            else
            {
        ...
        protected override void PersistUpdatedItem(IDataType entity)
        {
            ...
            if (entity.IsPropertyDirty("ParentId"))
            {
                ...
                var maxSortOrder =
                    Database.ExecuteScalar<int>(
                        "SELECT coalesce(max(sortOrder),0) FROM sbnNode WHERE parentid = @ParentId AND nodeObjectType = @NodeObjectType",
                        new { ParentId = entity.ParentId, NodeObjectType = NodeObjectTypeId });
            ...
            //Updates the (base) node data - sbnNode
            var nodeDto = dto.NodeDto;
            Database.Update(nodeDto);
            Database.Update(dto);
