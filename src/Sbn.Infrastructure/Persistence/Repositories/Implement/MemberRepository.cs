    ...
    public class MemberRepository : ContentRepositoryBase<int, IMember, MemberRepository>, IMemberRepository
    {
        ...
        protected override IEnumerable<IMember> PerformGetByQuery(IQuery<IMember> query)
        {
            ...
            if (wheres.Any(x => x.Item1.Contains("cmsPropertyType")))
            {
                ...
                baseQuery.Append("WHERE sbnNode.id IN (" + sql.SQL + ")", sql.Arguments)
        ...
        protected override IEnumerable<string> GetDeleteClauses()
        {
            ...
            {
                "DELETE FROM sbnUser2NodeNotify WHERE nodeId = @id",
                "DELETE FROM sbnUserGroup2NodePermission WHERE nodeId = @id",
                "DELETE FROM sbnRelation WHERE parentId = @id",
                "DELETE FROM sbnRelation WHERE childId = @id",
                ...
                "DELETE FROM sbnNode WHERE id = @id"
            };
            return list;
        }
        ...
        public IEnumerable<IMember> GetByMemberGroup(string groupName)
        {
            ...
            var sql = GetBaseQuery(false)
                ...
                .Append("WHERE sbnNode.id IN (" + subQuery.SQL + ")", subQuery.Arguments)
        ...
        public int GetCountByQuery(IQuery<IMember> query)
        {
            ...
            var fullSql = GetBaseQuery(true)
                .Append(new Sql("WHERE sbnNode.id IN (" + sql.SQL + ")", sql.Arguments));
        ...
        public void SetLastLogin(string username, DateTime date)
        {
            ...
            // to update there can be problems. This is only an issue for cmsPropertyData, not sbnContentVersion because that table just
            ...
            var sqlSelectTemplateProperty = SqlContext.Templates.Get("Sbn.Core.MemberRepository.SetLastLogin1", s => s
            ...
            // Update the sbnContentVersion value for the member

            var sqlSelectTemplateVersion = SqlContext.Templates.Get("Sbn.Core.MemberRepository.SetLastLogin2", s => s
