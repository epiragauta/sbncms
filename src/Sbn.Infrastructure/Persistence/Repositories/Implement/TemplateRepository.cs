...
namespace Sbn.Cms.Infrastructure.Persistence.Repositories.Implement
{
    ...
    internal class TemplateRepository : EntityRepositoryBase<int, ITemplate>, ITemplateRepository
    {
        ...
        protected override IEnumerable<ITemplate> PerformGetAll(params int[] ids)
        {
            ...
            if (ids.Any())
            {
                sql.Where("sbnNode.id in (@ids)", new { ids });
            }
            else
            {
        ...
        protected override IEnumerable<ITemplate> PerformGetByQuery(IQuery<ITemplate> query)
        {
            ...
            //look up the simple template definitions that have a master template assigned, this is used
            // later to populate the template item's properties
            ISbnEntity[] childIds = GetAxisDefinitions(dtos.ToArray()).ToArray();
        ...
        protected override void PersistUpdatedItem(ITemplate entity)
        {
            ...
            IEnumerable<ISbnEntity> axisDefs = GetAxisDefinitions(dto);
        ...
        private IEnumerable<ISbnEntity> GetAxisDefinitions(params TemplateDto[] templates)
        {
            //look up the simple template definitions that have a master template assigned, this is used
            // later to populate the template item's properties
            Sql<ISqlContext> childIdsSql = SqlContext.Sql()
                ...
                //lookup axis's
                .Where("sbnNode." + SqlContext.SqlSyntax.GetQuotedColumnName("id") + " IN (@parentIds) OR sbnNode.parentID IN (@childIds)",
        ...
        /// </param>
        /// <returns></returns>
        private ITemplate MapFromDto(TemplateDto dto, ISbnEntity[] axisDefinitions)
        {
            ...
            {
                ISbnEntity masterTemplate = axisDefinitions.FirstOrDefault(x => x.Id == dto.NodeDto.ParentId);
