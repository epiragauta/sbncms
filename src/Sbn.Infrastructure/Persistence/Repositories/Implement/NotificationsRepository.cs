    ...
    public class NotificationsRepository : INotificationsRepository
    {
        ...
        public IEnumerable<Notification> GetUsersNotifications(IEnumerable<int> userIds, string action, IEnumerable<int> nodeIds, Guid objectType)
        {
            ...
            var sql = AmbientScope.SqlContext.Sql()
                    .Select("DISTINCT sbnNode.id nodeId, sbnUser.id userId, sbnNode.nodeObjectType, sbnUser2NodeNotify.action")
        ...
        {
            var sql = AmbientScope.SqlContext.Sql()
                .Select("DISTINCT sbnNode.id AS nodeId, sbnUser2NodeNotify.userId, sbnNode.nodeObjectType, sbnUser2NodeNotify.action")
                .From<User2NodeNotifyDto>()
                .InnerJoin<NodeDto>()
