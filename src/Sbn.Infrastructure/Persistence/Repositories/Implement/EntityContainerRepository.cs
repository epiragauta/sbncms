...
namespace Sbn.Cms.Infrastructure.Persistence.Repositories.Implement
{
    ...
    internal class EntityContainerRepository : EntityRepositoryBase<int, EntityContainer>, IEntityContainerRepository
    {
        ...
        {
            var sql = GetBaseQuery(false).Where("text=@name AND level=@level AND nodeObjectType=@sbnObjectTypeId", new { name, level, sbnObjectTypeId = NodeObjectTypeId });
        ...
        protected override IEnumerable<EntityContainer> PerformGetAll(params int[] ids)
        {
            ...
            var sql = GetBaseQuery(false)
                .Where("nodeObjectType=@sbnObjectTypeId", new { sbnObjectTypeId = NodeObjectTypeId })
        ...
        protected override string GetBaseWhereClause() => "sbnNode.id = @id and nodeObjectType = @NodeObjectType";
