    ...
    internal class RelationRepository : EntityRepositoryBase<int, IRelation>, IRelationRepository
    {
        ...
        protected override IEnumerable<string> GetDeleteClauses()
        {
            var list = new List<string>
                           {
                               "DELETE FROM sbnRelation WHERE id = @id"
                           };
            return list;
        }
        ...
        private void SqlJoinRelations(Sql<ISqlContext> sql)
        {
            ...
            // both sides of the relation type unless the ISbnEntity query passed in filters one side out).
        ...
        public IEnumerable<ISbnEntity> GetPagedParentEntitiesByChildId(int childId, long pageIndex, int pageSize, out long totalRecords, params Guid[] entityTypes)
        {
            ...
            return _entityRepository.GetPagedResultsByQuery(Query<ISbnEntity>(), entityTypes, pageIndex, pageSize, out totalRecords, null, null, sql =>
            {
        ...
        public IEnumerable<ISbnEntity> GetPagedChildEntitiesByParentId(int parentId, long pageIndex, int pageSize, out long totalRecords, params Guid[] entityTypes)
        {
            // var contentObjectTypes = new[] { Constants.ObjectTypes.Document, Constants.ObjectTypes.Media, Constants.ObjectTypes.Member }
            // we could pass in the contentObjectTypes so that the entity repository sql is configured to do full entity lookups so that we get the full data
            // required to populate content, media or members, else we get the bare minimum data needed to populate an entity. BUT if we do this it
