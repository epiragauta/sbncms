...
using Sbn.Cms.Core.Persistence;
...
namespace Sbn.Cms.Infrastructure.Persistence.Repositories.Implement
{
        ...
        where TRepository : class, IRepository
    {
        ...
        private Sql<ISqlContext> PreparePageSql(Sql<ISqlContext> sql, Sql<ISqlContext> filterSql, Ordering ordering)
        {
            ...
            // no matter what we always MUST order the result also by sbnNode.id to ensure that all records being ordered by are unique.
            ...
            // is empty for many nodes) - see: http://issues.sbn.org/issue/U4-8831
            ...
            where T : class, IContentBase
        {
            ...
            // which is done when rebuilding caches/indexes/etc... in bulk. We are using an "IN" query on sbnPropertyData.VersionId
            // which then performs a Clustered Index Scan on PK_sbnPropertyData which means it iterates the entire table which can be enormous!
            // especially if there are both a lot of content but worse if there is a lot of versions of that content.
            // So is it possible to return this property data without doing an index scan on PK_sbnPropertyData and without iterating every row
            // in the table?
        ...
        private string GetAliasedField(string field, Sql sql)
        {
            ...
            //
            // so... if query contains "[sbnNode].[nodeId] AS [sbnNode__nodeId]"
            // then GetAliased for "[sbnNode].[nodeId]" returns "[sbnNode__nodeId]"
        ...
        protected void PersistRelations(TEntity entity)
        {
            ...
            var trackedRelations = new List<SbnEntityReference>();
