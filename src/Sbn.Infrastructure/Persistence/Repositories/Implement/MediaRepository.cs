    ...
    public class MediaRepository : ContentRepositoryBase<int, IMedia, MediaRepository>, IMediaRepository
    {
        ...
        public IMedia GetMediaByPath(string mediaPath)
        {
            ...
            if (isResized)
            {
                ...
                sbnFileValue = string.Concat(mediaPath.Substring(0, underscoreIndex), mediaPath.Substring(dotIndex));
            }

            var sql = GetBaseQuery(QueryType.Single, joinMediaVersion: true)
                .Where<MediaVersionDto>(x => x.Path == sbnFileValue)
                .SelectTop(1);
