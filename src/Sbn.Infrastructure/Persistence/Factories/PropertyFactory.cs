    ...
    internal static class PropertyFactory
    {
        ...
        private static PropertyDataDto BuildDto(int versionId, IProperty property, int? languageId, string segment, object value)
        {
            ...
            if (property.ValueStorageType == ValueStorageType.Integer)
            {
                ...
                else if (value != null && string.IsNullOrWhiteSpace(value.ToString()) == false && int.TryParse(value.ToString(), NumberStyles.Integer, CultureInfo.InvariantCulture, out var val))
                {
        ...
        /// The value of this will be used to populate the edited cultures in the sbnDocumentCultureVariation table.
        /// </param>
        /// <returns></returns>
            ...
            foreach (var property in properties)
            {
                if (property.PropertyType.SupportsPublishing)
                {
                    ...
                    foreach (var propertyValue in property.Values)
                    {
                        ...
                        // will end up incorrectly set in the sbnDocumentCultureVariation table so here we need to
