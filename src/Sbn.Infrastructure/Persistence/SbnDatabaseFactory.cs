...
using Sbn.Cms.Infrastructure.Persistence.FaultHandling;
...
namespace Sbn.Cms.Infrastructure.Persistence
{
    ...
    /// <summary>
    /// Default implementation of <see cref="ISbnDatabaseFactory"/>.
    /// </summary>
    /// <remarks>
    ...
    /// <para>It wraps an NPoco SbnDatabaseFactory which is initializes with a proper IPocoDataFactory to ensure
    ...
    public class SbnDatabaseFactory : DisposableObjectSlim, ISbnDatabaseFactory
    {
        ...
        private readonly ILogger<SbnDatabaseFactory> _logger;
        ...
        #region Constructors
          ...
          /// <summary>
        /// Initializes a new instance of the <see cref="SbnDatabaseFactory"/>.
        /// </summary>
        /// <remarks>Used by the other ctor and in tests.</remarks>
          internal SbnDatabaseFactory(
            ILogger<SbnDatabaseFactory> logger,
            ...
            string connectionString)
        {
            ...
            // could as well be <add name="sbnDbDSN" connectionString="" providerName="" />
        ...
        public SbnDatabaseFactory(
            ...
            DatabaseSchemaCreatorFactory databaseSchemaCreatorFactory,
            NPocoMapperCollection npocoMappers):
            this(logger, loggerFactory, globalSettings, mappers, dbProviderFactoryCreator, databaseSchemaCreatorFactory, npocoMappers, connectionStrings?.CurrentValue?.SbnConnectionString?.ConnectionString)
        {
        ...
        private SqlContext Initialize()
        {
            ...
            _npocoDatabaseFactory = DatabaseFactory.Config(x => x
                .UsingDatabase(CreateDatabaseInstance) // creating SbnDatabase instances
                .WithFluentConfig(config)); // with proper configuration

            if (_npocoDatabaseFactory == null)
            {
                throw new NullReferenceException("The call to SbnDatabaseFactory.Config yielded a null SbnDatabaseFactory instance.");
            }
        ...
        /// <inheritdoc />
        public ISbnDatabase CreateDatabase()
        {
            // must be initialized to create a database
            EnsureInitialized();
            return (ISbnDatabase) _npocoDatabaseFactory.GetDatabase();
        }
        ...
        private InitializedPocoDataBuilder GetPocoDataFactoryResolver(Type type, IPocoDataFactory factory)
            => new SbnPocoDataBuilder(type, _pocoMappers, _upgrading).Init();



        // method used by NPoco's SbnDatabaseFactory to actually create the database instance
        private SbnDatabase CreateDatabaseInstance()
            => new SbnDatabase(
                ConnectionString,
                SqlContext,
                DbProviderFactory,
                _loggerFactory.CreateLogger<SbnDatabase>(),
        ...
        protected override void DisposeResources()
        {
            ...
            //var db = _sbnDatabaseAccessor.SbnDatabase;
            //_sbnDatabaseAccessor.SbnDatabase = null;
            //db?.Dispose();
