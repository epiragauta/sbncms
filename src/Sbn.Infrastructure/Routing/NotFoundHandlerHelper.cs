...
{
    /// <summary>
    /// Used to determine the node to display when content is not found based on the configured error404 elements in sbnSettings.config
    /// </summary>
    internal class NotFoundHandlerHelper
    {
            ...
            int? domainContentId)
        {
            ...
            if (errorPage.HasContentKey)
            {
                ...
                Attempt<int> found = entityService.GetId(errorPage.ContentKey, SbnObjectTypes.Document);
                if (found)
                {
                    return found.Result;
                }
            ...
            if (errorPage.ContentXPath.IsNullOrWhiteSpace() == false)
            {
                try
                {
                    ...
                    var xpathResult = SbnXPathPathSyntaxParser.ParseXPathQuery(
