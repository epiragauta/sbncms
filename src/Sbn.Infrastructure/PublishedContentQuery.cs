...
namespace Sbn.Cms.Infrastructure
{
    /// <summary>
    ...
    public class PublishedContentQuery : IPublishedContentQuery
    {
        ...
        public IEnumerable<PublishedSearchResult> Search(string term, string culture = "*",
            string indexName = Constants.SbnIndexes.ExternalIndexName) =>
            Search(term, 0, 0, out _, culture, indexName);

        /// <inheritdoc />
        public IEnumerable<PublishedSearchResult> Search(string term, int skip, int take, out long totalRecords, string culture = "*", string indexName = Constants.SbnIndexes.ExternalIndexName, ISet<string> loadedFields = null)
        {
            if (skip < 0)
            {
            ...
            {
                indexName = Constants.SbnIndexes.ExternalIndexName;
            }

            if (!_examineManager.TryGetIndex(indexName, out var index) || !(index is ISbnIndex umbIndex))
            {
                throw new InvalidOperationException(
                    $"No index found by name {indexName} or is not of type {typeof(ISbnIndex)}");
            }
            ...
            else if (string.IsNullOrWhiteSpace(culture))
            {
                ...
                queryExecutor = query
                    .Field(SbnExamineFieldNames.VariesByCultureFieldName, "n") // Must not vary by culture
