...
namespace Sbn.Cms.Infrastructure.Macros
{
    /// <summary>
    ...
    public class MacroTagParser
    {
        ...
        ///     {/div}
        ///
        /// What this method will do is remove the {div} and parse out the commented special macro syntax: {?SBN_MACRO macroAlias=\"myMacro\" /}
        ...
        public static string FormatRichTextContentForPersistence(string rteContent)
        {
            ...
            var commentNodes = html.DocumentNode.SelectNodes("//comment()[contains(., '<?SBN_MACRO')]");
            ...
            Action<string, Dictionary<string, string>> macroFoundCallback )
        {
            ...
            while (!stop)
            {
                var tagIndex = fieldResult.ToString().ToLower().IndexOf("<?sbn");
                if (tagIndex > -1)
                {
                    ...
                    if (tag.Substring(tag.Length - 2, 1) != "/" && tag.IndexOf(" ") > -1)
                    {
                        ...
                        // Tag with children are only used when a macro is inserted by the sbn-editor, in the
                        // following format: "<?SBN_MACRO ...><IMG SRC="..."..></?SBN_MACRO>", so we
