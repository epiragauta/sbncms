...
namespace Sbn.Cms.Core.Services.Implement
{
    ...
    public class FileService : RepositoryService, IFileService
    {
        ...
        private const string PartialViewHeader = "@inherits Sbn.Cms.Web.Common.Views.SbnViewPage";
        private const string PartialViewMacroHeader = "@inherits Sbn.Cms.Web.Common.Macros.PartialViewMacroPage";
        ...
        public Attempt<OperationResult<OperationResultType, ITemplate>> CreateTemplateForContentType(string contentTypeAlias, string contentTypeName, int userId = Constants.Security.SuperUserId)
        {
            var template = new Template(_shortStringHelper, contentTypeName,
                ...
                // This fixes: http://issues.sbn.org/issue/U4-7953
                contentTypeName);
            ...
            using (IScope scope = ScopeProvider.CreateScope())
            {
                ...
                Audit(AuditType.Save, userId, template.Id, ObjectTypes.GetName(SbnObjectTypes.Template));
                scope.Complete();
            }
        ...
        public void SaveTemplate(ITemplate template, int userId = Constants.Security.SuperUserId)
        {
            ...
            using (IScope scope = ScopeProvider.CreateScope())
            {
                ...
                Audit(AuditType.Save, userId, template.Id, SbnObjectTypes.Template.GetName());
                scope.Complete();
            }
        }
        ...
        public void SaveTemplate(IEnumerable<ITemplate> templates, int userId = Constants.Security.SuperUserId)
        {
            ...
            using (IScope scope = ScopeProvider.CreateScope())
            {
                ...
                Audit(AuditType.Save, userId, -1, SbnObjectTypes.Template.GetName());
                scope.Complete();
            }
        }
        ...
        public void DeleteTemplate(string alias, int userId = Constants.Security.SuperUserId)
        {
            using (IScope scope = ScopeProvider.CreateScope())
            {
                ...
                Audit(AuditType.Delete, userId, template.Id, ObjectTypes.GetName(SbnObjectTypes.Template));
                scope.Complete();
            }
        }
        ...
        {
            var snippetPath = _hostingEnvironment.MapPathContentRoot($"{Constants.SystemDirectories.Sbn}/PartialViewMacros/Templates/");
        ...
        internal Attempt<string> TryGetSnippetPath(string fileName)
        {
            ...
            var snippetPath = _hostingEnvironment.MapPathContentRoot($"{Constants.SystemDirectories.Sbn}/PartialViewMacros/Templates/{fileName}");
