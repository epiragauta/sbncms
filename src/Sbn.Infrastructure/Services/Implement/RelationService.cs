...
namespace Sbn.Cms.Core.Services.Implement
{
    public class RelationService : RepositoryService, IRelationService
    {
        ...
        /// <inheritdoc />
        public IEnumerable<IRelation> GetByParent(ISbnEntity parent) => GetByParentId(parent.Id);

        /// <inheritdoc />
        public IEnumerable<IRelation> GetByParent(ISbnEntity parent, string relationTypeAlias) => GetByParentId(parent.Id, relationTypeAlias);
        ...
        /// <inheritdoc />
        public IEnumerable<IRelation> GetByChild(ISbnEntity child) => GetByChildId(child.Id);

        /// <inheritdoc />
        public IEnumerable<IRelation> GetByChild(ISbnEntity child, string relationTypeAlias) => GetByChildId(child.Id, relationTypeAlias);
        ...
        /// <inheritdoc />
        public ISbnEntity GetChildEntityFromRelation(IRelation relation)
        {
            var objectType = ObjectTypes.GetSbnObjectType(relation.ChildObjectType);
            return _entityService.Get(relation.ChildId, objectType);
        }

        /// <inheritdoc />
        public ISbnEntity GetParentEntityFromRelation(IRelation relation)
        {
            var objectType = ObjectTypes.GetSbnObjectType(relation.ParentObjectType);
            return _entityService.Get(relation.ParentId, objectType);
        }

        /// <inheritdoc />
        public Tuple<ISbnEntity, ISbnEntity> GetEntitiesFromRelation(IRelation relation)
        {
            var childObjectType = ObjectTypes.GetSbnObjectType(relation.ChildObjectType);
            var parentObjectType = ObjectTypes.GetSbnObjectType(relation.ParentObjectType);
            ...
            return new Tuple<ISbnEntity, ISbnEntity>(parent, child);
        }

        /// <inheritdoc />
        public IEnumerable<ISbnEntity> GetChildEntitiesFromRelations(IEnumerable<IRelation> relations)
        {
            ...
            foreach (var groupedRelations in relations.GroupBy(x => ObjectTypes.GetSbnObjectType(x.ChildObjectType)))
            {
        ...
        /// <inheritdoc />
        public IEnumerable<ISbnEntity> GetParentEntitiesFromRelations(IEnumerable<IRelation> relations)
        {
            // Trying to avoid full N+1 lookups, so we'll group by the object type and then use the GetAll
            // method to lookup batches of entities for each parent object type

            foreach (var groupedRelations in relations.GroupBy(x => ObjectTypes.GetSbnObjectType(x.ParentObjectType)))
            {
                var objectType = groupedRelations.Key;
        ...
        /// <inheritdoc />
        public IEnumerable<ISbnEntity> GetPagedParentEntitiesByChildId(int id, long pageIndex, int pageSize, out long totalChildren, params SbnObjectTypes[] entityTypes)
        {
            using (var scope = ScopeProvider.CreateScope(autoComplete: true))
            {
        ...
        /// <inheritdoc />
        public IEnumerable<ISbnEntity> GetPagedChildEntitiesByParentId(int id, long pageIndex, int pageSize, out long totalChildren, params SbnObjectTypes[] entityTypes)
        {
            using (var scope = ScopeProvider.CreateScope(autoComplete: true))
            {
        ...
        /// <inheritdoc />
        public IEnumerable<Tuple<ISbnEntity, ISbnEntity>> GetEntitiesFromRelations(IEnumerable<IRelation> relations)
        {
            //TODO: Argh! N+1
            ...
            foreach (var relation in relations)
            {
                ...
                yield return new Tuple<ISbnEntity, ISbnEntity>(parent, child);
            }
        }
        ...
        /// <inheritdoc />
        public IRelation Relate(ISbnEntity parent, ISbnEntity child, IRelationType relationType)
        {
        ...
        /// <inheritdoc />
        public IRelation Relate(ISbnEntity parent, ISbnEntity child, string relationTypeAlias)
        {
            var relationType = GetRelationTypeByAlias(relationTypeAlias);
            if (relationType == null || string.IsNullOrEmpty(relationType.Alias))
                throw new ArgumentNullException(string.Format("No RelationType with Alias '{0}' exists.", relationTypeAlias));
        ...
        /// <inheritdoc />
        public bool AreRelated(ISbnEntity parent, ISbnEntity child)
        {
            return AreRelated(parent.Id, child.Id);
        }

        /// <inheritdoc />
        public bool AreRelated(ISbnEntity parent, ISbnEntity child, string relationTypeAlias)
        {
        ...
        {
            _auditRepository.Save(new AuditItem(objectId, type, userId, ObjectTypes.GetName(SbnObjectTypes.RelationType), message));
        }
        #endregion
    }
}
