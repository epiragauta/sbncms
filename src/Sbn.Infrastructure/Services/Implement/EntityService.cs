...
namespace Sbn.Cms.Core.Services.Implement
{
    public class EntityService : RepositoryService, IEntityService
    {
        ...
        private readonly Dictionary<string, SbnObjectTypes> _objectTypes;
        private IQuery<ISbnEntity> _queryRootEntity;
            ...
            : base(provider, loggerFactory, eventMessagesFactory)
        {
            ...
            _objectTypes = new Dictionary<string, SbnObjectTypes>
            {
                { typeof (IDataType).FullName, SbnObjectTypes.DataType },
                { typeof (IContent).FullName, SbnObjectTypes.Document },
                { typeof (IContentType).FullName, SbnObjectTypes.DocumentType },
                { typeof (IMedia).FullName, SbnObjectTypes.Media },
                { typeof (IMediaType).FullName, SbnObjectTypes.MediaType },
                { typeof (IMember).FullName, SbnObjectTypes.Member },
                { typeof (IMemberType).FullName, SbnObjectTypes.MemberType },
            };
        }
        ...
        private IQuery<ISbnEntity> QueryRootEntity => _queryRootEntity
            ?? (_queryRootEntity = Query<ISbnEntity>().Where(x => x.ParentId == -1));

        #endregion

        // gets the object type, throws if not supported
        private SbnObjectTypes GetObjectType(Type type)
        {
        ...
        /// <inheritdoc />
        public virtual IEntitySlim Get(int id, SbnObjectTypes objectType)
        {
            using (ScopeProvider.CreateScope(autoComplete: true))
            {
        ...
        /// <inheritdoc />
        public IEntitySlim Get(Guid key, SbnObjectTypes objectType)
        {
            using (ScopeProvider.CreateScope(autoComplete: true))
            {
        ...
        /// <inheritdoc />
        public virtual IEnumerable<IEntitySlim> GetAll<T>() where T : ISbnEntity
        ...
        /// <inheritdoc />
        public virtual IEnumerable<IEntitySlim> GetAll(SbnObjectTypes objectType)
            => GetAll(objectType, Array.Empty<int>());

        /// <inheritdoc />
        public virtual IEnumerable<IEntitySlim> GetAll(SbnObjectTypes objectType, params int[] ids)
        {
        ...
        /// <inheritdoc />
        public IEnumerable<IEntitySlim> GetAll(SbnObjectTypes objectType, Guid[] keys)
        {
            var entityType = objectType.GetClrType();
            GetObjectType(entityType);
        ...
        /// <inheritdoc />
        public virtual IEnumerable<IEntitySlim> GetRootEntities(SbnObjectTypes objectType)
        {
            using (ScopeProvider.CreateScope(autoComplete: true))
            {
        ...
        /// <inheritdoc />
        public virtual IEntitySlim GetParent(int id, SbnObjectTypes objectType)
        {
            using (ScopeProvider.CreateScope(autoComplete: true))
            {
                var entity = _entityRepository.Get(id);
        ...
        public virtual IEnumerable<IEntitySlim> GetChildren(int parentId)
        {
            using (ScopeProvider.CreateScope(autoComplete: true))
            {
                var query = Query<ISbnEntity>().Where(x => x.ParentId == parentId);
                return _entityRepository.GetByQuery(query);
            }
        }

        /// <inheritdoc />
        public virtual IEnumerable<IEntitySlim> GetChildren(int parentId, SbnObjectTypes objectType)
        {
            using (ScopeProvider.CreateScope(autoComplete: true))
            {
                var query = Query<ISbnEntity>().Where(x => x.ParentId == parentId);
        ...
        public virtual IEnumerable<IEntitySlim> GetDescendants(int id)
        {
            using (ScopeProvider.CreateScope(autoComplete: true))
            {
                ...
                var query = Query<ISbnEntity>().Where(x => x.Path.StartsWith(pathMatch) && x.Id != id);
                return _entityRepository.GetByQuery(query);
            }
        }

        /// <inheritdoc />
        public virtual IEnumerable<IEntitySlim> GetDescendants(int id, SbnObjectTypes objectType)
        {
            using (ScopeProvider.CreateScope(autoComplete: true))
            {
                var entity = _entityRepository.Get(id);
                var query = Query<ISbnEntity>().Where(x => x.Path.StartsWith(entity.Path) && x.Id != id);
                return _entityRepository.GetByQuery(query, objectType.GetGuid());
            }
        }

        /// <inheritdoc />
        public IEnumerable<IEntitySlim> GetPagedChildren(int id, SbnObjectTypes objectType, long pageIndex, int pageSize, out long totalRecords,
            IQuery<ISbnEntity> filter = null, Ordering ordering = null)
        {
            using (ScopeProvider.CreateScope(autoComplete: true))
            {
                var query = Query<ISbnEntity>().Where(x => x.ParentId == id && x.Trashed == false);
        ...
        /// <inheritdoc />
        public IEnumerable<IEntitySlim> GetPagedDescendants(int id, SbnObjectTypes objectType, long pageIndex, int pageSize, out long totalRecords,
            IQuery<ISbnEntity> filter = null, Ordering ordering = null)
        {
            using (ScopeProvider.CreateScope(autoComplete: true))
            {
                var objectTypeGuid = objectType.GetGuid();
                var query = Query<ISbnEntity>();
        ...
        /// <inheritdoc />
        public IEnumerable<IEntitySlim> GetPagedDescendants(IEnumerable<int> ids, SbnObjectTypes objectType, long pageIndex, int pageSize, out long totalRecords,
            IQuery<ISbnEntity> filter = null, Ordering ordering = null)
        {
            totalRecords = 0;
            ...
            using (ScopeProvider.CreateScope(autoComplete: true))
            {
                ...
                if (idsA.All(x => x != Cms.Core.Constants.System.Root))
                {
                    ...
                    var clauses = new List<Expression<Func<ISbnEntity, bool>>>();
        ...
        /// <inheritdoc />
        public IEnumerable<IEntitySlim> GetPagedDescendants(SbnObjectTypes objectType, long pageIndex, int pageSize, out long totalRecords,
        ...
        /// <inheritdoc />
        public virtual SbnObjectTypes GetObjectType(int id)
        {
            using (var scope = ScopeProvider.CreateScope(autoComplete: true))
            {
                ...
                return ObjectTypes.GetSbnObjectType(guid);
            }
        }

        /// <inheritdoc />
        public virtual SbnObjectTypes GetObjectType(Guid key)
        {
            using (var scope = ScopeProvider.CreateScope(autoComplete: true))
            {
                var sql = scope.SqlContext.Sql()
        ...
        /// <inheritdoc />
        public virtual SbnObjectTypes GetObjectType(ISbnEntity entity)
        {
            return entity is IEntitySlim light
                ? ObjectTypes.GetSbnObjectType(light.NodeObjectType)
        ...
        /// <inheritdoc />
        public Attempt<int> GetId(Guid key, SbnObjectTypes objectType)
        {
        ...
        /// <inheritdoc />
        public Attempt<Guid> GetKey(int id, SbnObjectTypes sbnObjectType)
        {
            return _idKeyMap.GetKeyForId(id, sbnObjectType);
        }

        /// <inheritdoc />
        public virtual IEnumerable<TreeEntityPath> GetAllPaths(SbnObjectTypes objectType, params int[] ids)
        {
            var entityType = objectType.GetClrType();
            GetObjectType(entityType);
        ...
        /// <inheritdoc />
        public virtual IEnumerable<TreeEntityPath> GetAllPaths(SbnObjectTypes objectType, params Guid[] keys)
        {
            var entityType = objectType.GetClrType();
            GetObjectType(entityType);
