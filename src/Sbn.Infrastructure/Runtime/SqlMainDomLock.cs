...
using MapperCollection = Sbn.Cms.Infrastructure.Persistence.Mappers.MapperCollection;
...
namespace Sbn.Cms.Infrastructure.Runtime
{
    public class SqlMainDomLock : IMainDomLock
    {
            ...
            string connectionStringName)
        {
            ...
            _dbFactory = new SbnDatabaseFactory(
        ...
        : this(
            ...
            databaseSchemaCreatorFactory,
            npocoMappers,
            connectionStrings.CurrentValue.SbnConnectionString.ConnectionString
            )
        {
        ...
        private bool? TryAcquire(ISbnDatabase db, string tempId, string updatedTempId)
        {
            ...
            try
            {
                ...
                _sqlServerSyntax.ReadLock(db, Cms.Core.Constants.Locks.MainDom);
                        ...
                        // the row                        
                        var mainDomRows = db.Fetch<KeyValueDto>("SELECT * FROM sbnKeyValue WHERE [key] = @key", new { key = MainDomKey });
        ...
        private bool AcquireWhenMaxWaitTimeElapsed(ISbnDatabase db)
        {
            // Creates a separate transaction to the DB instance so we aren't allocating tons of new DB instances for each transaction
            // since this is executed in a tight loop
        ...
        /// </summary>
        private RecordPersistenceType InsertLockRecord(string id, ISbnDatabase db)
        {
        ...
        /// </summary>
        /// <returns></returns>
        private bool IsMainDomValue(string val, ISbnDatabase db)
        {
            return db.ExecuteScalar<int>("SELECT COUNT(*) FROM sbnKeyValue WHERE [key] = @key AND [value] = @val",
                        ...
                        if (_dbFactory.Configured && _hasTable)
                        {
                            ...
                            try
                            {
                                ...
                                if (_mainDomChanging)
                                {
                                    ...
                                    var count = db.Execute($"UPDATE sbnKeyValue SET [value] = [value] + '{UpdatedSuffix}' WHERE [key] = @key", new { key = MainDomKey });
                                }
                                else
                                {
                                    _logger.LogDebug("Releasing MainDom, deleting row, application is shutting down.");
                                    var count = db.Execute("DELETE FROM sbnKeyValue WHERE [key] = @key", new { key = MainDomKey });
                                }
                            }
                            catch (Exception ex)
                            {
