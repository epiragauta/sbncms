...
namespace Sbn.Cms.Core.Models.Mapping
{
    public class EntityMapDefinition : IMapDefinition
    {
        ...
        // Sbn.Code.MapAll -Trashed -Alias -Score
        ...
        private static void Map(ISearchResult source, SearchResultEntity target, MapperContext context)
        {
            ...
            target.Icon = source.Values.ContainsKey(SbnExamineFieldNames.IconFieldName)
                ? source.Values[SbnExamineFieldNames.IconFieldName]
                : Constants.Icons.DefaultIcon;

            target.Name = source.Values.ContainsKey(SbnExamineFieldNames.NodeNameFieldName) ? source.Values[SbnExamineFieldNames.NodeNameFieldName] : "[no name]";
            ...
            if (source.Values.TryGetValue(SbnExamineFieldNames.SbnFileFieldName, out var sbnFile) &&
                sbnFile.IsNullOrWhiteSpace() == false)
            {
                if (sbnFile != null)
                {
                    target.Name = $"{target.Name} ({sbnFile})";
                }
            }

            if (source.Values.ContainsKey(SbnExamineFieldNames.NodeKeyFieldName))
            {
                if (Guid.TryParse(source.Values[SbnExamineFieldNames.NodeKeyFieldName], out var key))
                {
                    target.Key = key;
            ...
            target.Path = source.Values.ContainsKey(SbnExamineFieldNames.IndexPathFieldName) ? source.Values[SbnExamineFieldNames.IndexPathFieldName] : "";
