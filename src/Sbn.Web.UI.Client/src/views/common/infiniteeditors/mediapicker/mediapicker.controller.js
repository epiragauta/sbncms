...
angular.module("sbn")
    .controller("Sbn.Editors.MediaPickerController",
        function ($scope, $timeout, mediaResource, entityResource, userService, mediaHelper, mediaTypeHelper, eventsService, treeService, localStorageService, localizationService, editorService, umbSessionStorage, notificationsService, clipboardService) {
            ...
            var sbnSettings = Sbn.Sys.ServerVariables.sbnSettings;
            var allowedUploadFiles = mediaHelper.formatFileTypes(sbnSettings.allowedUploadFiles);

            if ($scope.onlyImages) {
                vm.acceptedFileTypes = mediaHelper.formatFileTypes(sbnSettings.imageFileTypes);
            } else {
                ...
                if (allowedUploadFiles !== '') {
                    ...
                    vm.acceptedFileTypes = !mediaHelper.formatFileTypes(sbnSettings.disallowedUploadFiles);
                }
            }

            vm.maxFileSize = sbnSettings.maxFileSize + "KB";
            ...
            function setMediaMetaData(mediaItem) {
                ...
                if (mediaItem.metaData) {
                    ...
                    if (mediaItem.metaData.sbnWidth && mediaItem.metaData.sbnHeight) {
                        mediaItem.properties.push(
                            {
                                alias: "sbnWidth",
                                editor: mediaItem.metaData.sbnWidth.PropertyEditorAlias,
                                value: mediaItem.metaData.sbnWidth.Value
                            },
                            {
                                alias: "sbnHeight",
                                editor: mediaItem.metaData.sbnHeight.PropertyEditorAlias,
                                value: mediaItem.metaData.sbnHeight.Value
                            }
                        );
                    }
                    if (mediaItem.metaData.sbnFile) {
                        // this is required for resolving files through the mediahelper
                        mediaItem.properties.push(
                            {
                                alias: "sbnFile",
                                editor: mediaItem.metaData.sbnFile.PropertyEditorAlias,
                                value: mediaItem.metaData.sbnFile.Value
                            }
                        );
                    }
