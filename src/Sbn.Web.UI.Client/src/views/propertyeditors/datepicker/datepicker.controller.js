function dateTimePickerController($scope, angularHelper, dateHelper, validationMessageService) {
    ...
    function onInit() {
        ...
        if (Sbn.Sys.ServerVariables.application.serverTimeOffset !== undefined) {
    ...
    function updateModelValue(momentDate) {
        if ($scope.hasDatetimePickerValue) {
            if ($scope.model.config.pickTime) {
                ...
                if ($scope.model.value && Object.toBoolean($scope.model.config.offsetTime) && Sbn.Sys.ServerVariables.application.serverTimeOffset !== undefined) {
                    $scope.model.value = dateHelper.convertToServerStringTime(momentDate, Sbn.Sys.ServerVariables.application.serverTimeOffset);
                    $scope.serverTime = dateHelper.convertToServerStringTime(momentDate, Sbn.Sys.ServerVariables.application.serverTimeOffset, "YYYY-MM-DD HH:mm:ss Z");
                }
                else {
    ...
    function setDatePickerVal() {
        if ($scope.model.value) {
            ...
            if ($scope.model.value && Object.toBoolean($scope.model.config.offsetTime) && $scope.serverTimeNeedsOffsetting) {
                ...
                dateVal = dateHelper.convertToLocalMomentTime($scope.model.value, Sbn.Sys.ServerVariables.application.serverTimeOffset);
                $scope.serverTime = dateHelper.convertToServerStringTime(dateVal, Sbn.Sys.ServerVariables.application.serverTimeOffset, "YYYY-MM-DD HH:mm:ss Z");
            }
            else {
...
angular.module("sbn").controller("Sbn.PropertyEditors.DatepickerController", dateTimePickerController);
