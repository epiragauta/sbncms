...
(function() {
    ...
    function endPreviewSession() {
        ...
        if(amountOfPreviewSessions <= 0) {
            ...
            navigator.sendBeacon(scriptElement.getAttribute("data-sbn-path")+"/preview/end");
        }
    }
    startPreviewSession();
    ...
    function endPreviewMode() {
        ...
        window.top.location.href = scriptElement.getAttribute("data-sbn-path")+"/preview/end?redir=" + encodeURIComponent(window.location.pathname+window.location.search);
    }
    ...
    function askToViewPublishedVersion() {
        ...
        scriptElement.getAttribute("data-sbn-path");

        const request = new XMLHttpRequest();
        request.open("GET", scriptElement.getAttribute("data-sbn-path") + "/LocalizedText");
        request.send();
    ...
    function createAskUserAboutVersionDialog(jsonLocalization) {
        ...
        var modelStyles = `
            ...
            .sbn-preview-dialog__question {
                margin-bottom:30px;
            }
            .sbn-preview-dialog__modal > button {
            ...
            .sbn-preview-dialog__modal > button:hover {
                color: #2152a3;
                background-color: #f6f1ef;
            }
        ...
        con.className = "sbn-preview-dialog";
        shadowRoot.appendChild(con);

        var modal = document.createElement("div");
        modal.className = "sbn-preview-dialog__modal";
        modal.innerHTML = `<div class="sbn-preview-dialog__headline">${umbLocalizedVars.preview.viewPublishedContentHeadline}</div>
            <div class="sbn-preview-dialog__question">${umbLocalizedVars.preview.viewPublishedContentDescription}</div>`;
        con.appendChild(modal);

        var continueButton = document.createElement("button");
        continueButton.type = "button";
        continueButton.className = "sbn-preview-dialog__continue";
