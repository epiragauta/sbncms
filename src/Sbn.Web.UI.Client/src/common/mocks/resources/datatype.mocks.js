    ...
    factory('dataTypeMocks', ['$httpBackend', 'mocksUtils', function ($httpBackend, mocksUtils) {
        ...
        return {
            register: function () {
                ...
                $httpBackend
                    .whenPOST(mocksUtils.urlRegex('/sbn/SbnApi/DataType/PostSave'))
                    .respond(returnSave);

                $httpBackend
                    .whenGET(mocksUtils.urlRegex('/sbn/SbnApi/DataType/GetById'))
                    .respond(returnById);

                $httpBackend
                    .whenGET(mocksUtils.urlRegex('/sbn/SbnApi/DataType/GetEmpty'))
                    .respond(returnEmpty);

                $httpBackend
                    .whenGET(mocksUtils.urlRegex('/sbn/SbnApi/DataType/GetPreValues'))
                    .respond(returnPreValues);
            },
            expectGetById: function () {
                $httpBackend
                    .expectGET(mocksUtils.urlRegex('/sbn/SbnApi/DataType/GetById'));
            }
        };
    }]);
