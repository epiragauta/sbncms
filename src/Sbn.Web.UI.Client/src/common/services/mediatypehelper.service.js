/**
 * @ngdoc service
 * @name sbn.services.mediaTypeHelper
            ...
            return mediaTypeResource.getAllowedTypes(mediaId)
                .then(function(types){
                    ...
                    return $q.all(allowedQ).then(function(fullTypes){
                        ...
                        return mediaTypeHelperService.getTypeWithEditor(fullTypes, ['Sbn.ImageCropper', 'Sbn.UploadField']);
        ...
        getTypeAcceptingFileExtensions: function (mediaTypes, fileExtensions) {
            return mediaTypes.filter(mediaType => {
                ...
                mediaType.groups.forEach(group => {
                    var foundProperty = group.properties.find(property => property.alias === "sbnFile");
                    if(foundProperty) {
                ...
                if(uploadProperty) {
                    ...
                    if(uploadProperty.editor === "Sbn.ImageCropper") {
                        acceptedFileExtensions = Sbn.Sys.ServerVariables.sbnSettings.imageFileTypes;
                    } else if(uploadProperty.editor === "Sbn.UploadField") {
...
angular.module('sbn.services').factory('mediaTypeHelper', mediaTypeHelper);
