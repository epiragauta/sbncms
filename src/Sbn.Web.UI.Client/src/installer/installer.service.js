angular.module("sbn.install").factory('installerService', function ($rootScope, $q, $timeout, $http, $templateRequest){
	...
	//add to sbn installer facts here
	var facts = ["Sbn helped millions of people watch a man jump from the edge of space",
					"Over 500 000 websites are currently powered by Sbn",
					"At least 2 people have named their cat 'Sbn'",
					"On an average day more than 1000 people download Sbn",
					"<a target='_blank' rel='noopener' href='https://sbn.tv/'>sbn.tv</a> is the premier source of Sbn video tutorials to get you started",
					"You can find the world's friendliest CMS community at <a target='_blank' rel='noopener' href='https://our.sbn.com/'>our.sbn.com</a>",
					"You can become a certified Sbn developer by attending one of the official courses",
					"Sbn works really well on tablets",
					"You have 100% control over your markup and design when crafting a website in Sbn",
					"Sbn is the best of both worlds: 100% free and open source, and backed by a professional and profitable company",
					"There's a pretty big chance you've visited a website powered by Sbn today",
					"'Sbn-spotting' is the game of spotting big brands running Sbn",
					"At least 4 people have the Sbn logo tattooed on them",
					"'Sbn' is the Danish name for an allen key",
					"Sbn has been around since 2005, that's a looong time in IT",
					...
					"While you are installing Sbn someone else on the other side of the planet is probably doing it too",
					"You can extend Sbn without modifying the source code using either JavaScript or C#",
					"Sbn has been installed in more than 198 countries"
					 ];
    ...
    */
	...
	var service = {
		...
		//loads available packages from our.sbn.com
		getPackages : function(){
			return $http.get(Sbn.Sys.ServerVariables.installApiBaseUrl + "GetPackages");
		},

		getSteps : function(){
			return $http.get(Sbn.Sys.ServerVariables.installApiBaseUrl + "GetSetup");
		},
        ...
        */
		...
		install : function(){
			...
			function processInstallStep() {
                ...
                $http.post(Sbn.Sys.ServerVariables.installApiBaseUrl + "PostPerformInstall", _installerModel)
		...
		complete : function(){
            ...
            $http.post(Sbn.Sys.ServerVariables.installApiBaseUrl + "CompleteInstall", _installerModel)
                .then(function () {
                    service.status.progress = "100%";
                    service.status.done = true;
                    service.status.feedback = "Restarting and redirecting you to Sbn, please wait";
                    service.status.loading = false;
                    ...
                    window.location.href = Sbn.Sys.ServerVariables.sbnBaseUrl;
                }, handleErrorResponse);
	...
	var handleErrorResponse = function (response) {
        ...
        if (status >= 500 && status < 600) {
            ...
            // It might turn out that our new Api Convention `SbnApiBehaviorApplicationModelProvider` might handle this for us with it's custom error handling.
            $timeout(function () {
